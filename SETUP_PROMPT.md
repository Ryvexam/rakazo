# Set up Ryvoko with a coding agent
...
```text
Set up Ryvoko from published GHCR images and leave the web UI running.
...
1. Which directory should contain the Ryvoko folder (or use the current directory)?
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Ryvoko onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
...
2. Download and inspect this installer (do not clone the repository):
   https://raw.githubusercontent.com/Ryvexam/ryvoko/main/infra/compose/install-images.sh
   If that host is unreachable, use a mirror URL (e.g. set `RYVOKO_INSTALLER_URL` to
   `https://example.com/mirror/ryvoko/infra/compose/install-images.sh` and curl that instead).
3. Run `bash install-images.sh --prepare-only`. It downloads the Compose and environment example
   ...
   Optional: set `RYVOKO_DOWNLOAD_BASE` to a generic mirror of `infra/compose`, and/or use
   `--local` / `RYVOKO_DOWNLOAD_SKIP_EXISTING=1` when Compose files are already present locally.
...
```text
Set up Ryvoko locally and leave it running in a usable state.

Repository: https://github.com/Ryvexam/ryvoko.git
...
1. Should you clone into the current directory, or what parent directory should contain `ryvoko`? If you are already inside a Ryvoko checkout, offer to use it without recloning.
2. How should models be connected?
   - Add a deployment-wide `OPENROUTER_API_KEY` to `.env`.
   - Connect during Ryvoko onboarding with a provider API key or with ChatGPT Plus/Pro, GitHub Copilot, or SuperGrok / X Premium.
   - Defer model setup and verify infrastructure only. Make clear that bots cannot answer until a model is connected.
...
- If I requested Electron, leave the web stack running and then launch `pnpm --filter @ryvoko/desktop dev`. Verify the shell loads the same app. Let me make the Docker-versus-This-Mac choice because This Mac grants bots access under my OS account.
