...
[![GitHub stars](https://img.shields.io/github/stars/Ryvexam/ryvoko?labelColor=black&style=for-the-badge&color=2563EB)](https://github.com/Ryvexam/ryvoko/stargazers)
...
![Ryvoko — AI teammates you actually own](./docs/readme-hero.png)

Ryvoko is an open-source platform for running persistent AI teammates. It is available on the web,
...
Ryvoko is in beta. Learn more at [ryvoko.com](https://ryvoko.com).
...
The installer downloads the Compose files, creates `.env` with random secrets, and starts Ryvoko.
...
On restricted networks, override the installer download base (`RYVOKO_DOWNLOAD_BASE`), skip
existing Compose files (`--local` / `RYVOKO_DOWNLOAD_SKIP_EXISTING`), or mirror the bootstrap
script URL — see
...
# edit .env: SANDBOX_PROVIDER=box (or e2b / daytona) with its API key, RYVOKO_HOST=your.domain
bash install-images.sh
```
...
```bash
git clone https://github.com/Ryvexam/ryvoko.git
cd ryvoko
cp .env.example .env
```
...
The Electron and Expo apps are clients of the same Ryvoko API used by the web app.

With the development stack running, launch Electron with:

```bash
pnpm --filter @ryvoko/desktop dev
```

On first run the desktop app asks whether to run Ryvoko on this computer or connect to an existing
...
app verifies Ryvoko's health endpoint before saving, and later launches go straight to that
instance. The stack keeps running after the app quits; **Stop Local Stack** in the application
menu turns it off.

Use **Change Ryvoko Server…** in the application menu to reconnect. Closing that window without
saving returns to the previous instance. For development automation, set `RYVOKO_WEB_URL` to point
the shell somewhere else without changing the saved instance, or `RYVOKO_FORCE_SETUP=1` to run
setup again.
...
Ryvoko is licensed under the [Apache License 2.0](./LICENSE).

Questions and ideas are welcome in the [Ryvoko Discord community](https://discord.gg/RWwKa2Sn7h).
