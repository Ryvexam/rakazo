...
until "${compose[@]}" exec -T postgres pg_isready -U ryvoko >/dev/null 2>&1; do
  sleep 1
done
...
"${compose[@]}" exec -T postgres psql -X -U ryvoko -d ryvoko \
  --set=ON_ERROR_STOP=on --single-transaction --file=- < "$SRC/ryvoko.sql"
