...
Ryvoko exists so that people can own AI teammates that persist, use computers, and finish work beyond a single chat.
...
A managed Ryvoko service may improve convenience, but it does not become a hidden dependency of the self-hosted core.
...
Ryvoko gives each concurrently active Team bot a separate live desktop and browser process when the provider supports it, and unsupported providers expose the limitation instead of silently coupling control.
...
Ryvoko is not a model provider, a hosted-only service, a general virtual-machine manager, a visual workflow builder, or a CI system.
Ryvoko does not promise that a trusted host computer or third-party destination is isolated from data deliberately sent to it.
