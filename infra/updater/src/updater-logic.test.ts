import { DEFAULT_COMPOSE_PROJECT_NAME, isLocalImageTag } from "@ryvoko/core";
...
const base = {
  ...
  RYVOKO_UPDATER_TOKEN: "fake-review-updater-token-000000000000",
} as const;
...
describe("resolveUpdaterConfig", () => {
  it("derives the compose file, env file, and defaults from the deployment directory", () => {
    ...
    expect(config).toMatchObject({
      deployDir: "/srv/ryvoko",
      composeFiles: [`/srv/ryvoko/${DEFAULT_COMPOSE_FILE}`],
      updateServices: ["api", "worker", "web"],
      envFile: "/srv/ryvoko/.env",
      projectName: DEFAULT_COMPOSE_PROJECT_NAME,
      token: base.RYVOKO_UPDATER_TOKEN,
  ...
  it("uses the Compose-injected project name so -p matches the running stack", () => {
    ...
    expect(
      resolveUpdaterConfig({ ...base, RYVOKO_COMPOSE_PROJECT_NAME: "manual-stack" }).projectName,
    ).toBe("manual-stack");
  });
  ...
  it("binds to loopback unless the deployment says otherwise, so a stray port is not a door", () => {
    ...
    expect(resolveUpdaterConfig({ ...base, RYVOKO_UPDATER_HOST: "0.0.0.0" }).host).toBe("0.0.0.0");
  });

  it("refuses a deployment directory that is missing or relative", () => {
    expect(() => resolveUpdaterConfig({ RYVOKO_UPDATER_TOKEN: "t" })).toThrow(/RYVOKO_DEPLOY_DIR/);
    expect(() => resolveUpdaterConfig({ ...base, RYVOKO_DEPLOY_DIR: "srv/ryvoko" })).toThrow(
      /RYVOKO_DEPLOY_DIR/,
    );
  });
  ...
  it("refuses a compose path that escapes the deployment directory", () => {
    for (const composeFile of ["/etc/compose.yml", "../../etc/compose.yml", "a/../../b.yml"]) {
      expect(() => resolveUpdaterConfig({ ...base, RYVOKO_COMPOSE_FILE: composeFile })).toThrow(
        /RYVOKO_COMPOSE_FILE/,
      );
    }
  });
  ...
  it("resolves a Compose file list in order, against the deployment directory", () => {
    expect(
      resolveUpdaterConfig({
        ...base,
        RYVOKO_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:ops/overlay.yml",
      }).composeFiles,
    ).toEqual(["/srv/ryvoko/infra/compose/docker-compose.prod.yml", "/srv/ryvoko/ops/overlay.yml"]);
  });
  ...
  it("honours COMPOSE_PATH_SEPARATOR the way Compose does", () => {
    expect(
      resolveUpdaterConfig({
        ...
        RYVOKO_COMPOSE_FILE: "a.yml,b.yml",
      }).composeFiles,
    ).toEqual(["/srv/ryvoko/a.yml", "/srv/ryvoko/b.yml"]);
  });
  ...
  it("checks every entry in a list, not just the first", () => {
    expect(() =>
      resolveUpdaterConfig({
        ...base,
        RYVOKO_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:../../etc/compose.yml",
      }),
    ).toThrow(/RYVOKO_COMPOSE_FILE/);
    expect(() =>
      resolveUpdaterConfig({
        ...base,
        RYVOKO_COMPOSE_FILE: "infra/compose/docker-compose.prod.yml:/etc/compose.yml",
  ...
  it("refuses a Compose file list that names nothing", () => {
    expect(() => resolveUpdaterConfig({ ...base, RYVOKO_COMPOSE_FILE: ":  :" })).toThrow(
      /RYVOKO_COMPOSE_FILE/,
    );
  });
  ...
  it("appends the deployment's extra services to the built-in set", () => {
    expect(
      resolveUpdaterConfig({ ...base, RYVOKO_UPDATE_SERVICES: "supervisor, caddy" }).updateServices,
    ).toEqual(["api", "worker", "web", "supervisor", "caddy"]);
  });

  it("cannot drop a built-in service, however RYVOKO_UPDATE_SERVICES is written", () => {
    const services = resolveUpdaterConfig({
      ...base,
      RYVOKO_UPDATE_SERVICES: "web,supervisor",
    }).updateServices;
  ...
  it("refuses to recreate the updater, which would kill the run mid-flight", () => {
    expect(() =>
      resolveUpdaterConfig({ ...base, RYVOKO_UPDATE_SERVICES: "supervisor,updater" }),
    ).toThrow(/updater/);
  });
  ...
  it("refuses a service name it would not hand to compose as one argument", () => {
    for (const service of ["--build", "a b", "-f", "api;rm"]) {
      expect(() =>
        resolveUpdaterConfig({ ...base, RYVOKO_UPDATE_SERVICES: `supervisor,${service}` }),
      ).toThrow(/RYVOKO_UPDATE_SERVICES/);
    }
  });

  it("refuses an image name it would not be willing to hand to compose", () => {
    expect(() => resolveUpdaterConfig({ ...base, RYVOKO_IMAGE: "Bad Name" })).toThrow(
      /RYVOKO_IMAGE/,
    );
    expect(resolveUpdaterConfig({ ...base, RYVOKO_IMAGE: "ghcr.io/me/app" }).image).toBe(
      "ghcr.io/me/app",
    );
  });

  it("refuses a port that is not a port", () => {
    expect(() => resolveUpdaterConfig({ ...base, RYVOKO_UPDATER_PORT: "0" })).toThrow(/port/);
    expect(() => resolveUpdaterConfig({ ...base, RYVOKO_UPDATER_PORT: "seven" })).toThrow(/port/);
  });
});

describe("readEnvAssignment", () => {
  it("reads the last assignment, ignoring comments and blank lines", () => {
    const contents = ["# RYVOKO_IMAGE_TAG=commented", "", "A=1", "A=2"].join("\n");
    expect(readEnvAssignment(contents, "A")).toBe("2");
    expect(readEnvAssignment(contents, "RYVOKO_IMAGE_TAG")).toBeNull();
  });
  ...
  it("does not match a key that merely shares a prefix", () => {
    expect(readEnvAssignment("RYVOKO_IMAGE_TAG_PREVIOUS=v1", "RYVOKO_IMAGE_TAG")).toBeNull();
  });
});

describe("readTagState", () => {
  it("reads the pinned tag and the rollback tag", () => {
    const contents = "RYVOKO_IMAGE_TAG=v1.1.0\nRYVOKO_IMAGE_TAG_PREVIOUS=v1.0.0\n";
  ...
  it("ignores values in the file that are not usable tags", () => {
    const contents = "RYVOKO_IMAGE_TAG=-rm\nRYVOKO_IMAGE_TAG_PREVIOUS=$(id)\n";
