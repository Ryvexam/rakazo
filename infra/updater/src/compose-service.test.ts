...
import { RECREATED_SERVICES } from "@ryvoko/core";
...
describe("the updater compose service", () => {
  ...
  it("is bind-mounted at the same path it has on the host", () => {
    const mount = (updater.volumes ?? []).find((volume) => volume.includes("RYVOKO_DEPLOY_DIR"));
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    const deployDir = "${RYVOKO_DEPLOY_DIR:-/srv/ryvoko}";
    ...
    expect(updater.environment?.RYVOKO_DEPLOY_DIR).toBe(deployDir);
  ...
  it("pins its own image tag separately from the application image", () => {
    expect(updater.image).toContain("RYVOKO_UPDATER_IMAGE_TAG");
    for (const service of RECREATED_SERVICES) {
      expect(compose.services[service]?.image).toContain("RYVOKO_IMAGE_TAG");
    }
  });

  it("uses the official registry namespace and digest-pins third-party runtime images", () => {
    expect(updater.image).toContain("ghcr.io/ryvexam/ryvoko/updater");
    expect(compose.services.api?.image).toContain("ghcr.io/ryvexam/ryvoko/app");
  ...
  it("lets a deployment name its own Compose files and extra services", () => {
    ...
    expect(updater.environment?.RYVOKO_COMPOSE_FILE).toBe(
      interpolated("RYVOKO_COMPOSE_FILE", "infra/compose/docker-compose.prod.yml"),
    );
    expect(updater.environment?.RYVOKO_UPDATE_SERVICES).toBe(
      interpolated("RYVOKO_UPDATE_SERVICES"),
    );
  ...
  it("injects the actual Compose project name into the updater container", () => {
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.COMPOSE_PROJECT_NAME).toBe("${COMPOSE_PROJECT_NAME:-ryvoko-prod}");
  });

  it("does not load the application env_file into the root-equivalent process", () => {
    expect(updater.env_file).toBeUndefined();
    // biome-ignore lint/suspicious/noTemplateCurlyInString: this is the literal Compose expression
    expect(updater.environment?.RYVOKO_UPDATER_TOKEN).toBe("${RYVOKO_UPDATER_TOKEN:-}");
  });
  ...
  it("does not let the api container reach the Docker socket to update itself", () => {
    ...
    expect(compose.services.api?.environment?.RYVOKO_UPDATER_URL).toBe("http://updater:7092");
  });
