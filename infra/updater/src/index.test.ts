...
import type { ServerUpdateRun } from "@ryvoko/contracts";
...
const app = createUpdaterApp(
  resolveUpdaterConfig({
    RYVOKO_DEPLOY_DIR: "/ryvoko-updater-tests-no-such-directory",
    RYVOKO_UPDATER_TOKEN: token,
  }),
);
...
async function deployment(env = "RYVOKO_IMAGE_TAG=v1.0.0\nRYVOKO_IMAGE_TAG_PREVIOUS=v0.9.0\n") {
  const deployDir = await mkdtemp(path.join(os.tmpdir(), "ryvoko-updater-test-"));
  temporaryDirectories.push(deployDir);
  await writeFile(path.join(deployDir, ".env"), env);
  return {
    deployDir,
    config: resolveUpdaterConfig({ RYVOKO_DEPLOY_DIR: deployDir, RYVOKO_UPDATER_TOKEN: token }),
  };
}
...
describe("updater HTTP surface", () => {
  ...
  it("re-validates the branch at its own boundary", async () => {
    const response = await app.request("/apply", {
      method: "POST",
      headers: authorized,
      body: JSON.stringify({ repoUrl: "https://github.com/Ryvexam/ryvoko", branch: "--exec=id" }),
    });
  ...
  it("refuses a fork build when the deployment has no checkout to build from", async () => {
    ...
    const response = await createUpdaterApp(fixture.config).request("/apply", {
      method: "POST",
      headers: authorized,
      body: JSON.stringify({ repoUrl: "https://github.com/someone/ryvoko", branch: "main" }),
    });
    expect(response.status).toBe(400);
    const payload = (await response.json()) as { error: string };
  ...
  it("refuses a rollback when no previous tag was recorded", async () => {
    const fixture = await deployment("RYVOKO_IMAGE_TAG=v1.0.0\n");
...
describe("updater orchestration", () => {
  it("serializes updates before their asynchronous preflight can race", async () => {
    ...
    const input = { repoUrl: "https://github.com/Ryvexam/ryvoko", branch: "main" };
  ...
  it("restores the prior image after a recreate fails health checks", async () => {
    ...
    const response = await request(subject, "/apply", {
      repoUrl: "https://github.com/Ryvexam/ryvoko",
      branch: "main",
    });
    const record = (await response.json()) as ServerUpdateRun;
    ...
    expect(composeUp[0]?.env?.RYVOKO_IMAGE_TAG).toBe(`sha-${targetCommit}`);
    expect(composeUp[1]?.env?.RYVOKO_IMAGE_TAG).toBe("v1.0.0");
    expect(await readFile(path.join(fixture.deployDir, ".env"), "utf8")).toContain(
      "RYVOKO_IMAGE_TAG=v1.0.0",
    );
  ...
  it("resets a fork checkout when recreate fails after the fast-forward", async () => {
    const fixture = await deployment("RYVOKO_IMAGE_TAG=local\n");
    await mkdir(path.join(fixture.deployDir, ".git"));
    const calls: string[][] = [];
    let upCalls = 0;
    const run: UpdaterCommandRunner = async (command, args) => {
  ...
  it("refuses to replace a symlinked deployment environment", async () => {
    ...
    await writeFile(target, "RYVOKO_IMAGE_TAG=v1.0.0\n");
    await rm(envFile);
    ...
    expect(await readFile(target, "utf8")).toBe("RYVOKO_IMAGE_TAG=v1.0.0\n");
  });
...
describe("child process environment", () => {
  it("declares the deployment directory safe so git works on a non-root checkout", () => {
    ...
    const env = commandEnvironment({ RYVOKO_DEPLOY_DIR: "/srv/ryvoko" });
    expect(env.GIT_CONFIG_COUNT).toBe("1");
    expect(env.GIT_CONFIG_KEY_0).toBe("safe.directory");
    expect(env.GIT_CONFIG_VALUE_0).toBe("/srv/ryvoko");
  });

  it("keeps the exemption scoped to the deployment directory, whoever calls it", () => {
    const env = commandEnvironment(
      { RYVOKO_DEPLOY_DIR: "/opt/ryvoko" },
      { GIT_CONFIG_VALUE_0: "*", GIT_CONFIG_COUNT: "9" },
    );
    expect(env.GIT_CONFIG_VALUE_0).toBe("/opt/ryvoko");
    expect(env.GIT_CONFIG_COUNT).toBe("1");
  });
