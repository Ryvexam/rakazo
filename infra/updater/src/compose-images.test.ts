...
const FIRST_PARTY_IMAGE = /ghcr\.io\/ryvexam\/ryvoko\/([a-z0-9][a-z0-9._-]*)/g;
...
describe("the images compose file", () => {
  it("runs postgres, app roles, supervisor, and a published computer image", () => {
    ...
    for (const service of appServices) {
      expect(compose.services[service]?.image).toContain("ghcr.io/ryvexam/ryvoko/app");
      expect(compose.services[service]?.image).toContain("RYVOKO_IMAGE_TAG");
    }
    expect(compose.services.computer?.image).toContain("ghcr.io/ryvexam/ryvoko/computer");
    expect(compose.services.computer?.image).toContain("RYVOKO_COMPUTER_IMAGE_TAG");
  ...
  it("skips non-string Compose environment scalars when collecting image names", () => {
    ...
    expect(firstPartyImageNames("ghcr.io/ryvexam/ryvoko/computer:edge")).toEqual(["computer"]);
  });
  ...
  it("publishes the web UI on loopback only", () => {
    expect(compose.services.web?.ports).toEqual(["127.0.0.1:${RYVOKO_WEB_PORT:-5173}:5173"]);
    expect(compose.services.api?.ports).toEqual(["127.0.0.1:${RYVOKO_API_PORT:-3100}:3100"]);
