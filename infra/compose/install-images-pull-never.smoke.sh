...
[[ "$offline_out" == *"Ryvoko is starting"* ]] || fail "--offline did not start"
...
[[ "$old_out" != *"Ryvoko setup failed:"* ]] || fail "old Compose --offline should not hard-fail: $old_out"
[[ "$old_out" == *"Ryvoko is starting"* ]] || fail "old Compose --offline should continue: $old_out"
