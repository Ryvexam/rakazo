...
DOWNLOAD_BASE="${RYVOKO_DOWNLOAD_BASE:-https://raw.githubusercontent.com/Ryvexam/ryvoko/main/infra/compose}"
...
case "$DOWNLOAD_BASE" in
  https://*) ;;
  *)
    echo "Ryvoko setup failed: RYVOKO_DOWNLOAD_BASE must use https." >&2
    exit 1
    ;;
esac
...
skip_existing=false
pull_never=false
if [[ "${RYVOKO_DOWNLOAD_SKIP_EXISTING:-}" == "1" ]]; then
  skip_existing=true
fi
if [[ "${RYVOKO_PULL_NEVER:-}" == "1" ]]; then
  pull_never=true
fi
...
services:
  api:
    environment:
      _RYVOKO_VALIDATE_POSTGRES_PASSWORD: ${POSTGRES_PASSWORD:?Set POSTGRES_PASSWORD in .env}
      _RYVOKO_VALIDATE_BETTER_AUTH_SECRET: ${BETTER_AUTH_SECRET:?Set BETTER_AUTH_SECRET in .env}
      _RYVOKO_VALIDATE_ENCRYPTION_KEY: ${ENCRYPTION_KEY:?Set ENCRYPTION_KEY in .env}
      _RYVOKO_VALIDATE_SCREEN_PROXY_SECRET: ${SCREEN_PROXY_SECRET:?Set SCREEN_PROXY_SECRET in .env}
      _RYVOKO_VALIDATE_SANDBOX_SUPERVISOR_TOKEN: ${SANDBOX_SUPERVISOR_TOKEN:?Set SANDBOX_SUPERVISOR_TOKEN in .env}
YAML
  then
...
if [[ "$prepare_only" == true ]]; then
  echo "Ryvoko files are ready. Edit .env, then run: bash install-images.sh"
  exit 0
fi
...
echo "Ryvoko is starting at http://127.0.0.1:5173"
