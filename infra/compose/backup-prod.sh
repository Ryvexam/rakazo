...
PROJECT_DIR="${RYVOKO_DEPLOY_DIR:-/srv/ryvoko}"
[[ "${PROJECT_DIR}" == /* ]] || { echo "RYVOKO_DEPLOY_DIR must be an absolute path" >&2; exit 1; }
COMPOSE_FILE="${PROJECT_DIR}/infra/compose/docker-compose.prod.yml"
ENV_FILE="${PROJECT_DIR}/.env"
BACKUP_ROOT="/var/backups/ryvoko"
...
"${compose[@]}" exec -T postgres pg_restore --list \
  < "${SNAPSHOT_DIR}/ryvoko.dump" >/dev/null
tar -tzf "${SNAPSHOT_DIR}/appdata.tgz" >/dev/null

sha256sum "${SNAPSHOT_DIR}/ryvoko.dump" "${SNAPSHOT_DIR}/appdata.tgz" \
...
echo "Verified Ryvoko backup written to ${SNAPSHOT_DIR}"
