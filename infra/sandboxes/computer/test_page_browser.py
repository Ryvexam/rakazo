...
loader = importlib.machinery.SourceFileLoader("page_browser", str(Path(__file__).with_name("ryvoko-page-browser")))
...
class PageBrowserTest(unittest.TestCase):
    def test_closed_stdin_cancels_helper_without_a_browser(self):
        with subprocess.Popen([sys.executable, str(Path(__file__).with_name("ryvoko-page-browser")), "snapshot", "{}"],
                              env={**os.environ, "RYVOKO_BROWSER_WATCH_STDIN": "1", "RYVOKO_CDP_PORT": "0"},
