...
void *ryvoko_xcapture_open(const char *display_name) {
...
int ryvoko_xcapture_copy(void *context, const unsigned char **rgb, int *width, int *height) {
...
int ryvoko_xcapture_png(void *context, const unsigned char **png_bytes, size_t *png_size,
    int *width, int *height) {
  XCapture *capture = context;
  if (!png_bytes || !png_size) return -1;
  const unsigned char *rgb = NULL;
  if (ryvoko_xcapture_copy(context, &rgb, width, height)) return -1;
...
int ryvoko_xcapture_damage(void *context, int *x, int *y, int *width, int *height) {
  XCapture *capture = context;
...
void ryvoko_xcapture_close(void *context) {
  XCapture *capture = context;
  if (!capture) return;
...
int ryvoko_xinput_argv(void *context, int argc, const char *const *argv) {
  XCapture *capture = context;
