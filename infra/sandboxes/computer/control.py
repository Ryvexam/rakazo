#!/usr/bin/env python3
"""Token-auth desktop control for the Ryvoko supervisor."""
...
TOKEN = os.environ.get("RYVOKO_COMPUTER_CONTROL_TOKEN", "")
...
class NativeCapture:
    ...
    def __init__(self, display):
        library = ctypes.CDLL("/usr/local/lib/libryvoko-xcapture.so")
        library.ryvoko_xcapture_open.argtypes = [ctypes.c_char_p]
        library.ryvoko_xcapture_open.restype = ctypes.c_void_p
        library.ryvoko_xcapture_png.argtypes = [
            ctypes.c_void_p,
        ...
        library.ryvoko_xcapture_png.restype = ctypes.c_int
        library.ryvoko_xcapture_damage.argtypes = [
            ctypes.c_void_p,
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
            ctypes.POINTER(ctypes.c_int),
        ...
        library.ryvoko_xcapture_damage.restype = ctypes.c_int
        library.ryvoko_xinput_argv.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_char_p)]
        library.ryvoko_xinput_argv.restype = ctypes.c_int
        context = library.ryvoko_xcapture_open(display.encode("utf-8"))
        if not context:
    ...
    def copy(self):
        ...
        if self.library.ryvoko_xcapture_png(
        ...
        changed = self.library.ryvoko_xcapture_damage(
    ...
    def act(self, argv):
        ...
        return self.library.ryvoko_xinput_argv(self.context, len(argv), encoded)
...
def control_command_index(argv):
    ...
    return 3 if len(argv) > 2 and argv[2].startswith("RYVOKO_BROWSER_PROFILE=") else 2
...
def allowed_control_argv(argv, display):
    ...
    command = argv[index]
    if index == 3:
        profile = argv[2].removeprefix("RYVOKO_BROWSER_PROFILE=")
        if not re.fullmatch(r"/home/ryvoko/\.browser-profiles/chromium-bot-[a-f0-9]{32}", profile):
            return False
        if command not in ("xdg-open", "ryvoko-browser"):
            return False
    if command == "xdotool":
