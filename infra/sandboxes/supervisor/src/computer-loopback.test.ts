...
import { resolveSupervisorToken } from "@ryvoko/core";
...
beforeEach(async () => {
  ...
  vi.resetAllMocks();
  vi.stubEnv("HOSTNAME", "");
  vi.stubEnv("DATA_DIR", "/tmp/ryvoko-loopback-test");
...
describe("computer loopback provision lifecycle", () => {
  ...
  it("rejects another computer identity without stopping its container", async () => {
    ...
    const container = {
      inspect: vi.fn().mockResolvedValue({
        Config: {
          Labels: { "ryvoko.managed": "true", "ryvoko.botId": "other", "ryvoko.spaceId": "other" },
        },
      }),
      stop: vi.fn(),
      exec: vi.fn(),
  ...
  it("rechecks stopped state after a concurrent stop owns the screen lock", async () => {
    ...
    const container = {
      inspect: vi.fn(async () => ({
        Config: {
          Labels: { "ryvoko.managed": "true", "ryvoko.botId": "bot", "ryvoko.spaceId": "space" },
        },
  ...
  it.each([
    ...
    if (resumed) {
      ...
      expect(options.HostConfig.Binds).toEqual([`${homePath}:/home/ryvoko`]);
      expect(options.Env).toContainEqual(
        expect.stringMatching(/^RYVOKO_COMPUTER_CONTROL_TOKEN=.+/),
      );
    }
  });
});
...
describe("space computer limit enforcement", () => {
  ...
  async function provisionBot(
    ...
    return supervisorApp.request("/computers", {
      ...
      headers: {
        authorization: `Bearer ${resolveSupervisorToken(process.env)}`,
        "content-type": "application/json",
        "x-ryvoko-bot-id": botId,
        "x-ryvoko-space-id": spaceId,
      },
  ...
  it("rejects new container creation with 429 when space container limit is reached", async () => {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        if (labels.some((l: string) => l.startsWith("ryvoko.botId="))) {
          return [];
        }
        // For countSpaceContainers
        return [
          { Id: "c1", Labels: { "ryvoko.managed": "true", "ryvoko.spaceId": "space-1" } },
          { Id: "c2", Labels: { "ryvoko.managed": "true", "ryvoko.spaceId": "space-1" } },
        ];
      },
    );
  ...
  it("allows new container creation when under space limit", async () => {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        return [{ Id: "c1", Labels: { "ryvoko.managed": "true", "ryvoko.spaceId": "space-1" } }];
      },
    );
  ...
  it("resumes existing container even if space is at limit", async () => {
    ...
    const existing = {
      ...
      inspect: vi.fn().mockResolvedValue({
        ...
        Config: {
          ...
          Labels: {
            "ryvoko.managed": "true",
            "ryvoko.botId": "bot-existing",
            "ryvoko.spaceId": "space-1",
          },
        },
        State: { Running: true },
        HostConfig: {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        const labels = opts?.filters?.label ?? [];
        if (labels.some((l: string) => l === "ryvoko.botId=bot-existing")) {
          return [
            {
              Id: existing.id,
              Labels: {
        ...
        return [
          { Id: existing.id, Labels: { "ryvoko.managed": "true", "ryvoko.spaceId": "space-1" } },
        ];
      },
    );
  ...
  it("counts legacy workspaceId COMPUTER_IMAGE containers toward the limit", async () => {
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        // Legacy managed computer: COMPUTER_IMAGE + workspaceId, no ryvoko.managed.
        return [
          {
            Id: "legacy",
            Image: COMPUTER_IMAGE,
            Labels: { "ryvoko.workspaceId": "space-1", "ryvoko.botId": "legacy-bot" },
          },
        ];
      },
    );
    ...
    mocks.docker.listContainers.mockImplementation(
      async (opts?: { filters?: { label?: string[] } }) => {
        ...
        return Array.from({ length: created }, (_, index) => ({
          Id: `c${index}`,
          Labels: { "ryvoko.managed": "true", "ryvoko.spaceId": "space-1" },
        }));
      },
    );
