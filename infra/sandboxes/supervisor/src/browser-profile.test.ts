...
function fixture() {
  const root = mkdtempSync(path.join(tmpdir(), "ryvoko-profile-test-"));
  fixtures.push(root);
  ...
  mkdirSync(runtime);
  const profile = (bot: string) => browserProfilePathForScreen(bot).replace("/home/ryvoko", home);
  const run = (script: string) => {
    ...
    return spawnSync(
      "bash",
      ["-eu", "-c", portable.replaceAll("/tmp/ryvoko", runtime).replaceAll("/home/ryvoko", home)],
      {
        encoding: "utf8",
        timeout: 10_000,
      },
