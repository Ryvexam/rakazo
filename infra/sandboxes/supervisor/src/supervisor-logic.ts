...
} from "@ryvoko/core/node/desktop-runtime";
...
import { canReleaseScreenLease, canTakeScreenLease } from "@ryvoko/core";
import { z } from "zod";
...
export { BROWSER_APPLICATIONS as DOCKER_BROWSER_ALIASES } from "@ryvoko/core/node/desktop-runtime";
...
export function hasComputerIdentity(
  ...
  const labeledSpaceId = labels?.["ryvoko.spaceId"] ?? labels?.["ryvoko.workspaceId"];
  return labels?.["ryvoko.botId"] === botId && labeledSpaceId === spaceId;
}
...
export function containerActionStep(
  ...
  if (action.kind === "key" || action.kind === "pointer" || action.kind === "clipboard") {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browserProfile ? [`RYVOKO_BROWSER_PROFILE=${browserProfile}`] : []),
      "xdg-open",
      target,
    ];
  } else {
    ...
    argv = [
      "env",
      `DISPLAY=${display}`,
      ...(browser && browserProfile ? [`RYVOKO_BROWSER_PROFILE=${browserProfile}`] : []),
      browser ? "ryvoko-browser" : action.application,
...
export const COMPUTER_WORKSPACE_ROOT = "/home/ryvoko";
...
export function workspaceFileAccessPython(): string {
  return [
    ...
    "    return os.path.realpath(os.fsdecode(fcntl.fcntl(fd, getpath)).rstrip('\\0'))",
