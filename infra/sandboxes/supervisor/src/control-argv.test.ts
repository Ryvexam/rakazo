...
import { browserProfilePathForScreen } from "@ryvoko/core/node/desktop-runtime";
  ...
  (browserProfile) => {
    it.each([
      ...
      if (
        ...
        expect(step.argv[2]).toBe(`RYVOKO_BROWSER_PROFILE=${browserProfile}`);
      }
...
describe("controller argv restrictions", () => {
  it.each([
    ["env", `DISPLAY=${display}`, `RYVOKO_BROWSER_PROFILE=${profile}`],
    ["env", `DISPLAY=${display}`, "RYVOKO_BROWSER_PROFILE=", "ryvoko-browser"],
    ["env", `DISPLAY=${display}`, `RYVOKO_BROWSER_PROFILE=${profile}a`, "ryvoko-browser"],
    [
      "env",
      `DISPLAY=${display}`,
      `RYVOKO_BROWSER_PROFILE=${profile.toUpperCase()}`,
      "ryvoko-browser",
    ],
    ["env", `DISPLAY=${display}`, `RYVOKO_BROWSER_PROFILE=${profile}`, "xterm"],
    [
      "env",
      `DISPLAY=${display}`,
      `RYVOKO_BROWSER_PROFILE=${profile}`,
      "/usr/bin/xdg-open",
      "https://example.com",
    ],
    ["env", `DISPLAY=${display}`, `RYVOKO_BROWSER_PROFILE=${profile}`, "xdg-open"],
    [
      "env",
      `DISPLAY=${display}`,
      `RYVOKO_BROWSER_PROFILE=${profile}`,
    ...
    ["env", "DISPLAY=:8", `RYVOKO_BROWSER_PROFILE=${profile}`, "ryvoko-browser"],
    ["env", "DISPLAY=:8", "xdg-open", "https://example.com"],
    ["env", `DISPLAY=${display}`, "LD_PRELOAD=/tmp/unsafe", "xdg-open", "https://example.com"],
    ["env", `DISPLAY=${display}`, "RYVOKO_BROWSER_PROFILE=/tmp/unsafe", "ryvoko-browser"],
    ["env", `DISPLAY=${display}`, `RYVOKO_BROWSER_PROFILE=${profile}/../other`, "ryvoko-browser"],
    ["env", `DISPLAY=${display}`, `RYVOKO_BROWSER_PROFILE=${profile}`, "sh", "-c", "true"],
    ["env", `DISPLAY=${display}`, `RYVOKO_BROWSER_PROFILE=${profile}`, "xdotool", "key", "Return"],
    [
      "env",
      `DISPLAY=${display}`,
      `RYVOKO_BROWSER_PROFILE=${profile}`,
