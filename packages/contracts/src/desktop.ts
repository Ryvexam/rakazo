...
export interface RyvokoDesktopUpdate {
...
export interface RyvokoDesktopOAuthCallback {
  code: string;
  state?: string;
}

export interface RyvokoDesktop {
  ...
  oauth: {
     ...
     */
    onCallback: (listener: (callback: RyvokoDesktopOAuthCallback) => void) => () => void;
  };
}

/**
 * How the desktop app was pointed at a Ryvoko server during first-run setup.
 ...
 * narrower `ryvokoDesktop` bridge so a connected server can never re-point the app.
 */
export interface RyvokoSetup {
