...
describe("resolveAutoReviewChecker", () => {
  it("prefers explicit env overrides", () => {
    expect(
      resolveAutoReviewChecker({
        RYVOKO_AUTO_REVIEW_PROVIDER: "openrouter",
        RYVOKO_AUTO_REVIEW_MODEL: "cheap/fast",
        RYVOKO_LOCAL_MODELS: "local-a",
  ...
  it("prefers local models when configured", () => {
    expect(
      resolveAutoReviewChecker({
        RYVOKO_LOCAL_MODELS: " llama-local , other ",
...
describe("isAutoReviewCheckerConfigured", () => {
  it("requires a runnable checker", () => {
    ...
    expect(
      isAutoReviewCheckerConfigured({
        env: { RYVOKO_LOCAL_MODELS: "local-1" },
      }),
    ).toBe(true);
    expect(
      isAutoReviewCheckerConfigured({
...
describe("deploymentAutoReviewDefault", () => {
  it("defaults off", () => {
    ...
    expect(deploymentAutoReviewDefault({ RYVOKO_AUTO_REVIEW: "1" })).toBe(true);
    expect(deploymentAutoReviewDefault({ RYVOKO_AUTO_REVIEW: "true" })).toBe(true);
  });
});
...
describe("autoReviewTimeoutMs", () => {
  it("defaults to 1500 and clamps bad values", () => {
    ...
    expect(autoReviewTimeoutMs({ RYVOKO_AUTO_REVIEW_TIMEOUT_MS: "2000" })).toBe(2_000);
    expect(autoReviewTimeoutMs({ RYVOKO_AUTO_REVIEW_TIMEOUT_MS: "nope" })).toBe(1_500);
  });
});
