...
import {
  DEFAULT_RYVOKO_EMULATED_RELEASES,
    ...
    messages: [
      {
        ...
        subject: "Welcome to Ryvoko",
        sender: "hello@ryvoko.test",
        to: "me@example.test",
  ...
  return [
    {
      ...
      inputSchema: {
        type: "object",
        properties: {
          owner: { type: "string", description: "Repository owner, e.g. Ryvexam" },
          repo: { type: "string", description: "Repository name, e.g. ryvoko" },
        },
        required: ["owner", "repo"],
      },
    },
...
export class ComposioEmulator implements ComposioProvider {
  ...
  private githubReleases: EmulatedGithubRelease[] = [...DEFAULT_RYVOKO_EMULATED_RELEASES];
  ...
  private executeGithub(tool: string, args: Record<string, unknown>): Record<string, unknown> {
    const owner = String(args.owner ?? args.owner_name ?? "Ryvexam");
    const repo = String(args.repo ?? args.repository ?? "ryvoko");
