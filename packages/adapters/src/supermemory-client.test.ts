...
describe("searchSupermemory", () => {
  it("posts the query and container tag, returning search results on success", async () => {
    ...
    const result = await searchSupermemory("spelling preference", "ryvoko:bot-123", config);
  ...
  it("reports a non-OK response instead of throwing", async () => {
    ...
    const result = await searchSupermemory("anything", "ryvoko:bot-123", config);
  ...
  it("treats document chunks as memory text when the search result has no memory field", async () => {
    ...
    const result = await searchSupermemory("database", "ryvoko:bot-123", config);
...
describe("searchSupermemoryContainers", () => {
  it("deduplicates and ranks results from shared and bot containers", async () => {
    ...
    await expect(
      searchSupermemoryContainers("query", ["ryvoko:workspace:ws-1", "ryvoko:bot-1"], config),
    ).resolves.toEqual({
      ok: true,
      results: [
        { memory: "private", similarity: 0.9 },
...
describe("saveSupermemoryMemory", () => {
  ...
  it("reports a non-OK response instead of throwing", async () => {
    ...
    const result = await saveSupermemoryMemory("fact", "ryvoko:bot-123", config);
...
describe("saveSupermemoryMemoryToContainers", () => {
  it("writes shared memories to both the workspace and bot containers", async () => {
    ...
    await expect(
      saveSupermemoryMemoryToContainers("fact", ["ryvoko:workspace:ws-1", "ryvoko:bot-1"], config),
...
describe("deleteSupermemoryContainer", () => {
  it("deletes the container tag on success", async () => {
    ...
    const result = await deleteSupermemoryContainer("ryvoko:bot-123", config);

    expect(result).toEqual({ ok: true });
    const [url, init] = fetchMock.mock.calls[0]!;
    expect(url).toBe("http://localhost:6767/v3/container-tags/ryvoko%3Abot-123");
