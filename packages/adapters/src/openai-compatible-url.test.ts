...
const savedAllowPublic = process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC;

afterEach(() => {
  if (savedAllowPublic === undefined) delete process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC;
  else process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC = savedAllowPublic;
});
...
describe("openai-compatible URL policy", () => {
  ...
  it("rejects public hosts unless explicitly allowed", () => {
    delete process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC;
