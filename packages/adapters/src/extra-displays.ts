import type { ComputerAction, ComputerInput } from "@ryvoko/adapter-kit";
...
export function observeExtraDisplayCommand(layout: ExtraDisplayLayout): string {
  const imagePath = `/tmp/ryvoko/observe-${layout.displayNumber}.png`;
  return [
    `DISPLAY=${layout.display} xdotool getmouselocation --shell >/tmp/ryvoko/cursor-${layout.displayNumber}.txt || true`,
    ...
    `printf '\\nCURSOR '`,
    `tr '\\n' ' ' </tmp/ryvoko/cursor-${layout.displayNumber}.txt 2>/dev/null || true`,
  ].join("; ");
}
