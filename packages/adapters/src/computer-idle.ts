...
import { ACTIVE_RUN_STATUSES } from "@ryvoko/core";
...
const BACKGROUND_WORK_MARKER_PREFIX = "/tmp/ryvoko-background-";
const BACKGROUND_WORK_IDLE_SENTINEL = "ryvoko-background-idle";
...
export const CANCEL_COMPUTER_RUN_WORK = [
  ...
  'pkill -TERM -f "ryvoko-background-launch ${computerId} ${runId} " 2>/dev/null || true',
  "if [ -d /proc ]; then",
  ...
  "fi",
  "sleep 0.2",
  'pkill -KILL -f "ryvoko-background-launch ${computerId} ${runId} " 2>/dev/null || true',
  "if [ -d /proc ]; then",
  "  for fd in /proc/[0-9]*/fd/*; do",
  '    target="$(readlink "$fd" 2>/dev/null)" || continue',
  '    case "$target" in',
...
export function cancelComputerRunWorkArgv(computerId: string, runId: string): string[] {
  return ["bash", "-c", CANCEL_COMPUTER_RUN_WORK, "ryvoko-cancel-run-work", computerId, runId];
}
...
async function hasActiveBackgroundWork(
  ...
  try {
    for await (const event of sandbox.execute(
      computer,
      {
        argv: ["bash", "-c", BACKGROUND_WORK_PROBE, "ryvoko-background-probe", computerId],
        timeoutMs: 10_000,
      },
      context,
    )) {
