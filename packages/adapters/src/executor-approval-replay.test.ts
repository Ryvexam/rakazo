import type { ConnectorTool } from "@ryvoko/adapter-kit";
import { approvalEffectKey } from "@ryvoko/core/node/approval-effect-key";
...
describe("executor approval replay", () => {
  ...
  it("replays an approved lazy tool through the exposed catalog executor", () => {
    ...
    expect(continuation).not.toContain("__ryvokoCatalogTool");
  });
  ...
  it("keeps a direct-tool argument named like the catalog marker in continuation JSON", () => {
      ...
      [
        {
          ...
          request: {
            ...
            __ryvokoCatalogTool: "installed_execute_tool",
          },
        },
      ],
      JSON.stringify,
    ...
    expect(continuation).toContain(
      'notes.write: {"id":"row-1","arguments":{"mode":"strict"},"text":"approved exactly","__ryvokoCatalogTool":"installed_execute_tool"}',
    );
  });
  ...
  it("keeps resolveCall parsed args when draining an approved catalog replay", () => {
    const marker = "__ryvokoCatalogTool";
  ...
  it("preserves direct approved args that use the catalog marker as data", () => {
    const approvedRequest = {
      __ryvokoCatalogTool: "user-provided-value",
      target: "approved",
    };
    ...
    expect(
      approvedReplayArgs(
        approvedRequest,
        { __ryvokoCatalogTool: "user-provided-value", target: "reconstructed" },
        "__ryvokoCatalogTool",
      ),
