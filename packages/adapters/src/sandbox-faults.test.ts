...
import type { PortableFile, SandboxProvider } from "@ryvoko/adapter-kit";
...
describe.each([
  ...
  [
    "desktop",
    async () => {
      const root = await mkdtemp(path.join(tmpdir(), "ryvoko-sandbox-faults-"));
...
async function providerSet(label: string): Promise<Array<[string, SandboxProvider]>> {
  const root = await mkdtemp(path.join(tmpdir(), `ryvoko-sandbox-transfer-${label}-`));
  temporaryRoots.push(root);
  return [
