...
import { boundedSandboxCommandTimeoutMs } from "@ryvoko/core";
...
const BOX_WORKSPACE = "/home/user/ryvoko-home";
...
export class BoxSandboxProvider implements SandboxProvider {
    ...
    const created = await this.client.create(
      {
        createBoxRequest: {
          ...
          noEnv: true,
          env: {
            RYVOKO_BOT_ID: request.botId,
            RYVOKO_SANDBOX: "computer",
          },
        },
      },
      { signal: context.signal },
  ...
  private async runCommand(
    ...
    const marker = `/tmp/ryvoko-command-${randomUUID()}.completed-124`;
  ...
  private async terminateCommand(id: string, marker: string): Promise<void> {
    const pattern = marker.replace("ryvoko", "[r]yvoko");
    await this.rawCommand(
      id,
...
function boxCwd(cwd: string | undefined): string {
  ...
  const relative = cwd.startsWith(`${BOX_WORKSPACE}/`)
    ...
    : cwd.startsWith("/home/ryvoko/")
      ? cwd.slice("/home/ryvoko/".length)
      : cwd;
  return path.posix.join("ryvoko-home", normalizeWorkspacePath(relative));
}
