...
describe("desktop sandbox path rules", () => {
  it("compares Windows roots case-insensitively without accepting siblings or other drives", () => {
    const roots = ["C:\\Users\\Owner\\Ryvoko\\bot"];

    expect(
      isAllowedDesktopPath("c:\\users\\owner\\ryvoko\\BOT\\notes.txt", roots, path.win32),
    ).toBe(true);
    expect(
      isAllowedDesktopPath("C:\\Users\\Owner\\Ryvoko\\bot-other\\notes.txt", roots, path.win32),
    ).toBe(false);
  ...
  it("handles explicitly allowed UNC roots without accepting sibling shares", () => {
    const roots = ["\\\\server\\share\\ryvoko\\bot"];

    expect(
      isAllowedDesktopPath("\\\\SERVER\\share\\ryvoko\\BOT\\notes.txt", roots, path.win32),
    ).toBe(true);
    expect(
      isAllowedDesktopPath("\\\\server\\share-other\\ryvoko\\bot\\notes.txt", roots, path.win32),
    ).toBe(false);
  });
