...
import type { ActionApprovalRule } from "@ryvoko/core";
import { type AutoReviewJudgeDecision, redactSecrets } from "@ryvoko/core";
...
function localModelIds(env: NodeJS.ProcessEnv): string[] {
  return (env.RYVOKO_LOCAL_MODELS ?? "")
    .split(",")
    .map((id) => id.trim())
...
export function deploymentAutoReviewDefault(env: NodeJS.ProcessEnv = process.env): boolean {
  return envFlag(env, "RYVOKO_AUTO_REVIEW");
}

export function autoReviewTimeoutMs(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.RYVOKO_AUTO_REVIEW_TIMEOUT_MS?.trim();
...
export function resolveAutoReviewChecker(
  ...
  const overrideProvider = env.RYVOKO_AUTO_REVIEW_PROVIDER?.trim();
  const overrideModel = env.RYVOKO_AUTO_REVIEW_MODEL?.trim();
