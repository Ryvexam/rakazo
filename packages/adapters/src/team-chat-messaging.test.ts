import type { MessagingInboundMessage, MessagingSurface } from "@ryvoko/adapter-kit";
...
describe("toTeamChatInbound", () => {
  ...
  it("treats @mentions as mention when kind is omitted", () => {
    expect(toTeamChatInbound(baseEvent({ content: "hey @ryvoko look" }))?.kind).toBe("mention");
  });
});
