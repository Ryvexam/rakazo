...
export class LinuxDesktop {
  ...
  private async ensure(computer: ComputerRef, context: AdapterContext) {
    ...
    const match = output.match(/RYVOKO_DESKTOP=(\d+):([a-zA-Z0-9_-]+)/);
...
function browserActionCommand(
  ...
  if (browser !== undefined) {
    return `nohup ${browserLauncherPath(layout.displayNumber)} ${shellQuote(browser)} </dev/null >/tmp/ryvoko/browser-open-${layout.displayNumber}.log 2>&1 &`;
  }
