...
import type { AgentHomeStore, JobPublisher, SandboxProvider } from "@ryvoko/adapter-kit";
import type { PrismaClient, ThreadEvents } from "@ryvoko/db";
  ...
  it("probes background work with the database computer id, not providerRef", async () => {
    ...
    expect(harness.sandbox.execute).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        argv: ["bash", "-c", BACKGROUND_WORK_PROBE, "ryvoko-background-probe", harness.computer.id],
      }),
      expect.anything(),
    );
...
describe("background work launch and probe", () => {
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      markers.add(`/tmp/ryvoko-background-${databaseId}-run-1-${launchId}`);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const completedMarker = `/tmp/ryvoko-background-${markerId}-run-1-completed`;
      const activeMarker = `/tmp/ryvoko-background-${markerId}-run-1-active`;
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const marker = `/tmp/ryvoko-background-${markerId}-run-1-collision`;
      const commandRan = `/tmp/ryvoko-background-command-ran-${markerId}`;
      markers.add(marker);
  ...
  it.skipIf(process.platform === "win32")(
    ...
    async () => {
      ...
      const launchId = "active";
      const marker = `/tmp/ryvoko-background-${computerId}-${runId}-${launchId}`;
      markers.add(marker);
      ...
      const cancel = spawn(
        "bash",
        ["-c", CANCEL_COMPUTER_RUN_WORK, "ryvoko-cancel-run-work", computerId, runId],
        { stdio: "ignore" },
      );
  ...
  const sandbox = {
    execute: vi.fn(async function* () {
      ...
      if (code === 1 && !options.backgroundWorkProbeFailed) {
        yield { type: "stdout", data: "ryvoko-background-idle\n" } as const;
      }
...
function probeBackgroundWork(markerId: string): Promise<number> {
  return new Promise((resolve, reject) => {
    const child = spawn(
      "bash",
      ["-c", BACKGROUND_WORK_PROBE, "ryvoko-background-probe", markerId],
