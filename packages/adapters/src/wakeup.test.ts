import type { BackgroundJobHandlers } from "@ryvoko/adapter-kit";
...
describe("InMemoryJobQueue", () => {
  ...
  it("keeps wrapped payloads readable by workers that do not unwrap envelopes", async () => {
    const { wrapJobPayload } = await import("@ryvoko/logging");
    const { parseBackgroundJob } = await import("@ryvoko/adapter-kit");
