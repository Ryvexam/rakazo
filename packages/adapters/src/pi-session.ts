 ...
 * This is deliberately a recorder only: Ryvoko remains responsible for running
...
export class PiJsonlSessionRecorder implements PiSessionRecorder {
  ...
  async start(input: PiSessionStart): Promise<PiSessionHandle> {
    ...
    await handle.appendCustomEntry("ryvoko_context", {
      ryvokoUserId: input.userId,
      ryvokoBotId: input.botId,
      ryvokoRunId: input.runId,
      ryvokoThreadId: input.threadId,
      ...(input.traceId ? { ryvokoTraceId: input.traceId } : {}),
      model: input.model,
