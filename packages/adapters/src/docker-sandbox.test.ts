import type { ProcessEvent } from "@ryvoko/adapter-kit";
...
describe("Docker sandbox", () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        headers: expect.objectContaining({
          ...
          "x-ryvoko-bot-id": "home-bot",
          "x-ryvoko-screen-id": "bot",
          "x-ryvoko-screen-lease-id": "run-1:1",
  ...
  it("asks the supervisor to cancel orphaned run work when releasing a screen", async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        method: "DELETE",
        headers: expect.objectContaining({ "x-ryvoko-cancel-run-work": "1" }),
      }),
    );
  });
});
