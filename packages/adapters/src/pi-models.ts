...
import type { ModelOAuthSignInMode, ThinkingLevel } from "@ryvoko/contracts";
...
function catalogBilling(
  ...
  if (providerId === LOCAL_PROVIDER_ID) {
    return "Runs on infrastructure configured by the deployment owner. No model charges from Ryvoko.";
  }
  if (providerId === OPENAI_COMPATIBLE_PROVIDER_ID) {
    return "Runs on a URL you control. Ryvoko does not pay for model usage.";
  }
  if (opts.oauth && !opts.apiKey) {
    return `${name} subscription login is not in the Ryvoko UI yet. Skip if this deployment already has credentials.`;
  }
  if (opts.apiKey) {
    return `Uses your ${name} API key. Ryvoko does not pay for model usage.`;
  }
  return `Uses your ${name} key. Ryvoko does not pay for model usage.`;
}
