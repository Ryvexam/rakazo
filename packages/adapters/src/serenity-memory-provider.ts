...
export function serenityBotEntity(botId: string, brainLabel = ""): string {
  ...
  return label ? `ryvoko-bot/${label}/${botId}` : `ryvoko-bot/${botId}`;
}

export function serenitySpaceEntity(spaceId: string, brainLabel = ""): string {
  const label = sanitizeSerenityBrainLabel(brainLabel);
  return label ? `ryvoko-space/${label}/${spaceId}` : `ryvoko-space/${spaceId}`;
}
...
export class SerenityMemoryProvider implements SemanticMemoryProvider {
  ...
  async recall(
    ...
    // History compaction stays in Ryvoko; Serenity is the durable brain only.
  ...
  async save(
    ...
    const provenance = `ryvoko space:${context.spaceId} bot:${request.botId}`;
    const results = await Promise.all(
      entities.map((entity) =>
