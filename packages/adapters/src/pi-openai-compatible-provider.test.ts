...
import { OPENAI_COMPATIBLE_PROVIDER_ID } from "@ryvoko/contracts";
...
describe("model connect", () => {
  ...
  it("rejects public http openai-compatible connects when an API key is set", () => {
    const previous = process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC;
    process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC = "1";
    try {
      expect(() =>
        prepareOpenAiCompatibleConnect({
          provider: OPENAI_COMPATIBLE_PROVIDER_ID,
      ...
      if (previous === undefined) delete process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC;
      else process.env.RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC = previous;
    }
  });
...
describe("openai-compatible provider", () => {
  ...
  it("lists openai-compatible in the catalog even without RYVOKO_LOCAL_MODELS", () => {
    delete process.env.RYVOKO_LOCAL_MODELS;
