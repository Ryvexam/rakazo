...
import type { SearchHit, ThreadSnapshot } from "@ryvoko/contracts";
...
describeSearch("workspace search", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-search-"));
  ...
  it("finds bots, messages, files, links, and routines within the workspace", async () => {
    const cookie = await signup(app, `search-${stamp}@ryvoko.test`, "Search User");
  ...
  it("returns no hits for another workspace", async () => {
    const ownerCookie = await signup(app, `search-owner-${stamp}@ryvoko.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `search-intruder-${stamp}@ryvoko.test`, "Intruder");
  ...
  it("finds group conversations, messages, and files", async () => {
    const cookie = await signup(app, `search-group-${stamp}@ryvoko.test`, "Group Search User");
