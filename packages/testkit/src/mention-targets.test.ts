...
describeWithDatabase("structured @ mention targets", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-mention-targets-"));
  ...
  it("starts a routine test run on the owning bot", async () => {
    const cookie = await signup(app, `mention-routine-${stamp}@ryvoko.test`, "Routine Owner");
  ...
  it("replays routine testRun with the same clientNonce", async () => {
    const cookie = await signup(app, `mention-routine-replay-${stamp}@ryvoko.test`, "Replay Owner");
    const bot = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "ReplayBot",
      title: "",
      description: "",
  ...
  it("includes connector intent on a 1:1 send prompt", async () => {
    const cookie = await signup(app, `mention-connector-${stamp}@ryvoko.test`, "Connector Owner");
    const bot = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "Chief",
      title: "",
      description: "",
  ...
  it("lands a group-targeted send in the group transcript, not the 1:1 bot thread", async () => {
    const cookie = await signup(app, `mention-group-${stamp}@ryvoko.test`, "Group Owner");
  ...
  it("wakes exactly one bot on an unmentioned group send", async () => {
    const cookie = await signup(app, `mention-group-default-${stamp}@ryvoko.test`, "Group Default");
    const botA = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "BotA",
      title: "",
      description: "",
  ...
  it("wakes a mentioned group member from typed bot chips and ignores non-members", async () => {
    const cookie = await signup(app, `mention-out-${stamp}@ryvoko.test`, "Out Of Chat");
    const botA = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "MemberA",
      title: "",
      description: "",
  ...
  it("rejects another user's routine, connection, and group mentions", async () => {
    const ada = await signup(app, `mention-auth-ada-${stamp}@ryvoko.test`, "Ada Auth");
    const bob = await signup(app, `mention-auth-bob-${stamp}@ryvoko.test`, "Bob Auth");
