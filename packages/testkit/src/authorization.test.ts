...
import { ComposioEmulator } from "@ryvoko/adapters";
import type { appContract, Space, SpaceNavigation } from "@ryvoko/contracts";
import {
...
describeWithDatabase("API authorization and resource isolation", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-authz-"));
  ...
  it("prevents one user from reading or mutating another user's resources", async () => {
    const owner = await signup(app, `owner-authz-${stamp}@ryvoko.test`, "Authorization Owner");
    const intruder = await signup(
      app,
      `intruder-authz-${stamp}@ryvoko.test`,
  ...
  it("keeps approval rules private to each user in a shared Space", async () => {
    const owner = await signup(app, `approval-owner-${stamp}@ryvoko.test`, "Approval Owner");
    const member = await signup(app, `approval-member-${stamp}@ryvoko.test`, "Approval Member");
    const ownerActor = await rpc<Actor>(app, owner, "me");
  ...
  it("keeps space data and computers behind the selected space boundary", async () => {
    const cookie = await signup(app, `spaces-${stamp}@ryvoko.test`, "Space Owner");
    ...
    const intruder = await signup(app, `spaces-intruder-${stamp}@ryvoko.test`, "Intruder");
  ...
  it("enforces the space limit across concurrent creation requests", async () => {
    const cookie = await signup(app, `space-limit-${stamp}@ryvoko.test`, "Space Limit");
  ...
  it("reuses provider credentials and copies their selections into a new Space", async () => {
    const cookie = await signup(app, `space-provider-copy-${stamp}@ryvoko.test`, "Provider Copy");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("shares model credentials while keeping defaults private to each space", async () => {
    const cookie = await signup(app, `model-defaults-${stamp}@ryvoko.test`, "Model Defaults");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("deletes only empty, non-default spaces", async () => {
    const cookie = await signup(app, `space-delete-${stamp}@ryvoko.test`, "Space Delete");
    const actor = await rpc<Actor>(app, cookie, "me");
    ...
    const intruder = await signup(app, `space-delete-intruder-${stamp}@ryvoko.test`, "Intruder");
    ...
    const memberCookie = await signup(
      app,
      `space-delete-member-${stamp}@ryvoko.test`,
      "Space Member",
    );
  ...
  it("blocks bot creation after empty space deletion is claimed", async () => {
    const cookie = await signup(app, `space-race-${stamp}@ryvoko.test`, "Space Race");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("keeps a space claimed after ambiguous sandbox teardown failure", async () => {
    const cookie = await signup(app, `space-teardown-${stamp}@ryvoko.test`, "Teardown");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("times out a hung sandbox teardown without unblocking the space", async () => {
    const cookie = await signup(app, `space-deadline-${stamp}@ryvoko.test`, "Deadline");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("validates custom thinking against the saved connection capability", async () => {
    const cookie = await signup(app, `custom-thinking-${stamp}@ryvoko.test`, "Custom Thinking");
  ...
  it("validates per-bot model overrides against connected providers and catalog", async () => {
    const cookie = await signup(app, `bot-model-${stamp}@ryvoko.test`, "Bot Model");
    const bot = await rpc<
      Bot & {
  ...
  it("chooses the newest duplicate provider credential when selecting a default", async () => {
    const cookie = await signup(app, `model-duplicates-${stamp}@ryvoko.test`, "Model Duplicates");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("restricts deployment settings to the deployment owner", async () => {
    const owner = await signup(app, `deployment-owner-${stamp}@ryvoko.test`, "Deployment Owner");
    const other = await signup(app, `deployment-other-${stamp}@ryvoko.test`, "Deployment Other");
    const ownerActor = await rpc<Actor>(app, owner, "me");
    ...
    try {
      ...
      const closedSignup = await app.request("/api/auth/sign-up/email", {
        ...
        body: JSON.stringify({
          email: `closed-${stamp}@ryvoko.test`,
      ...
      const disallowedSignup = await app.request("/api/auth/sign-up/email", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          email: `not-approved-${stamp}@ryvoko.test`,
          password: "password123",
