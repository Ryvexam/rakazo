...
describeRunsList("runs.list activity tracker", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-runs-list-"));
  ...
  it("lists active runs from two bots and keeps completed runs under recent", async () => {
    const cookie = await signup(app, `runs-${stamp}@ryvoko.test`, "Runs User");
  ...
  it("never returns runs from another workspace", async () => {
    const ownerCookie = await signup(app, `runs-owner-${stamp}@ryvoko.test`, "Owner");
    ...
    const intruderCookie = await signup(app, `runs-intruder-${stamp}@ryvoko.test`, "Intruder");
  ...
  it("excludes archived bots from recent before applying the limit", async () => {
    const cookie = await signup(app, `runs-archived-${stamp}@ryvoko.test`, "Archive User");
  ...
  it("returns group runs with groupId for navigation", async () => {
    const cookie = await signup(app, `runs-group-${stamp}@ryvoko.test`, "Group User");
