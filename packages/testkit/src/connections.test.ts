...
describeWithDatabase("Composio catalog reconciliation", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-connections-"));
  ...
  it("reconciles one scoped row per provider under concurrent catalog fetches", async () => {
    const ownerCookie = await signup(app, `owner-connections-${stamp}@ryvoko.test`, "Owner");
    const otherCookie = await signup(app, `other-connections-${stamp}@ryvoko.test`, "Other");
  ...
  it("keeps a second connected account for the same provider and renames it", async () => {
    const cookie = await signup(app, `multi-account-${stamp}@ryvoko.test`, "Multi Account");
  ...
  it("returns the remote catalog when local reconciliation fails", async () => {
    const cookie = await signup(app, `db-failure-connections-${stamp}@ryvoko.test`, "DB Failure");
  ...
  it("does not mutate local state when the provider catalog fails", async () => {
    const cookie = await signup(
      app,
      `provider-failure-connections-${stamp}@ryvoko.test`,
      "Provider Failure",
    );
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("routes an emulated Composio app tool with user-scoped connection context", async () => {
    const cookie = await signup(app, `composio-tool-${stamp}@ryvoko.test`, "Composio Tool");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("runs Pipedream connection and MCP tool execution through the product registry", async () => {
    const cookie = await signup(app, `pipedream-${stamp}@ryvoko.test`, "Pipedream Connector");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("installs Treg, Executor, and custom MCP sources and routes their calls", async () => {
    const cookie = await signup(app, `mcp-connectors-${stamp}@ryvoko.test`, "MCP Connectors");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("imports an OpenAPI connector, keeps its credential encrypted, and routes calls", async () => {
    const cookie = await signup(app, `api-connector-${stamp}@ryvoko.test`, "API Connector");
    const actor = await rpc<Actor>(app, cookie, "me");
  ...
  it("imports a GraphQL connector, introspects operations, and routes calls", async () => {
    const cookie = await signup(app, `graphql-connector-${stamp}@ryvoko.test`, "GraphQL Connector");
    const actor = await rpc<Actor>(app, cookie, "me");
    const install = await rpc<{
      id: string;
      config: Record<string, unknown>;
