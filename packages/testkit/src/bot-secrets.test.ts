...
import type { AgentRuntimeEvent } from "@ryvoko/adapter-kit";
import { ScriptedAgentRuntime } from "@ryvoko/adapters";
import { answerRunInput } from "@ryvoko/db";
...
describeIntegration("reusable credential lifecycle", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-secret-lifecycle-"));
