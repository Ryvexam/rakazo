...
import type { RunsListOutput, ThreadSnapshot } from "@ryvoko/contracts";
import { isTerminal } from "@ryvoko/core";
...
const project = `ryvoko-topology-${process.pid}-${Date.now().toString(36)}`;
...
async function main() {
  ...
  const env = {
    ...
    TOPOLOGY_APP_IMAGE: process.env.TOPOLOGY_APP_IMAGE ?? "ryvoko/app:topology",
    TOPOLOGY_COMPUTER_IMAGE: process.env.TOPOLOGY_COMPUTER_IMAGE ?? "ryvoko/computer:topology",
  };
...
async function signup(baseUrl: string) {
  const response = await fetch(`${baseUrl}/api/auth/sign-up/email`, {
    ...
    body: JSON.stringify({
      email: `topology-${Date.now()}@ryvoko.test`,
      password: "password12",
      name: "Topology",
    }),
  });
...
async function waitForWorker(
  ...
  while (Date.now() - started < timeoutMs) {
    ...
    // {"timestamp":...,"level":"info","message":"worker ready","service.name":"ryvoko-worker"},
    // not the old plain "ryvoko worker ready" console.log line -- match the JSON field directly
...
function assertComputerNetworkIsolation(
  ...
  const managed = docker([
    ...
    computerId,
    "--format",
    '{{index .Config.Labels "ryvoko.managed"}}',
  ]).trim();
