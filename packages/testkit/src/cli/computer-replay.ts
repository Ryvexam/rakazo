...
async function main() {
  ...
  if (unknown.length)
    throw new Error(
      "Usage: computer-replay.ts [--image=ryvoko/computer:local] [--subnet=unused-private-CIDR] [--live --record=new-fixture.json]",
    );
  const image = imageArg?.slice("--image=".length) || "ryvoko/computer:local";
  ...
  const directory = await mkdtemp(path.join(tmpdir(), "ryvoko-computer-replay-"));
