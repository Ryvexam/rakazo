...
async function main() {
  ...
  execFileSync("pnpm", ["--filter", "@ryvoko/db", "exec", "prisma", "migrate", "deploy"], {
    ...
    for (const flow of ["screenshots", "notification-demo"]) {
      await runProcess(
        ...
        [
          ...
          "-e",
          `RYVOKO_SCREENSHOT_EMAIL=${EMAIL}`,
          "-e",
          `RYVOKO_SCREENSHOT_PASSWORD=${PASSWORD}`,
          "-e",
          `RYVOKO_SCREENSHOT_BOT_ID=${fixture.botId}`,
          "-e",
          `RYVOKO_SCREENSHOT_GROUP_ID=${fixture.groupId}`,
          "-e",
          `RYVOKO_SCREENSHOT_ROUTINE_ID=${fixture.routineId}`,
          "-e",
          `RYVOKO_NOTIFICATION_VIDEO=${path.join(REPORT_DIR, "notification-demo")}`,
          "-e",
          `RYVOKO_EXPAND_NOTIFICATIONS_URL=${EXPAND_NOTIFICATIONS_URL}`,
