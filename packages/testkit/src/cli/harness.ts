...
async function main() {
  ...
  try {
    ...
    execSync("pnpm --filter @ryvoko/db generate", { stdio: "inherit", env: process.env });
    execSync("pnpm --filter @ryvoko/db exec prisma migrate deploy", {
      stdio: "inherit",
      env: process.env,
    ...
    const [
      ...
      { createApp },
    ] = await Promise.all([import("@ryvoko/adapters"), import("../../../../apps/api/src/app.ts")]);
