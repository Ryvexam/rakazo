...
import { ModelConnectInputSchema } from "@ryvoko/contracts";
...
async function main() {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-evals-"));
  ...
  try {
    ...
    execFileSync("pnpm", ["--filter", "@ryvoko/db", "generate"], {
      stdio: "pipe",
      timeout: 120_000,
    });
