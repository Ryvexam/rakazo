...
import { abortableDelay } from "@ryvoko/core";
import { loadRootEnv } from "@ryvoko/core/node/load-root-env";
import { createThreadMessage, type PrismaClient } from "@ryvoko/db";
...
const temporaryRoot = await mkdtemp(path.join(os.tmpdir(), "ryvoko-desktop-performance-"));
...
function performanceEnvironment(databaseUrl: string): NodeJS.ProcessEnv {
  return {
    ...
    E2B_API_KEY: "",
    DAYTONA_API_KEY: "",
    BETTER_AUTH_SECRET: "ryvoko-benchmark-auth-secret-over-32-characters",
    ENCRYPTION_KEY: "ryvoko-benchmark-encryption-key-over-32-characters",
    SANDBOX_SUPERVISOR_TOKEN: "ryvoko-benchmark-supervisor-token-over-32-characters",
    SCREEN_PROXY_SECRET: "ryvoko-benchmark-screen-proxy-secret-over-32-characters",
    ...
    RYVOKO_HOST: "127.0.0.1",
    RYVOKO_WEB_URL: webOrigin,
    RYVOKO_DISABLE_BUNDLED_RENDERER: remoteRenderer ? "1" : "0",
    RYVOKO_DISABLE_WARM_WINDOW: disableWarmWindow ? "1" : "0",
    RYVOKO_PERFORMANCE_ASSET_DELAY_MS: String(assetDelayMs),
...
function buildProductionArtifacts(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@ryvoko/desktop", "pack:dir"], env);
}

function migrateDatabase(env: NodeJS.ProcessEnv) {
  run("pnpm", ["--filter", "@ryvoko/db", "generate"], env);
  run("pnpm", ["--filter", "@ryvoko/db", "exec", "prisma", "migrate", "deploy"], env);
}
...
async function packagedExecutable() {
  ...
  const candidates = process.platform === "darwin" ? await findNamed(out, "Ryvoko.app") : [];
  if (process.platform === "darwin" && candidates[0]) {
    return path.join(candidates[0], "Contents/MacOS/Ryvoko");
  }
...
async function prepareAuthenticatedProfile(benchmark: BenchmarkContext, profile: string) {
  ...
  try {
    ...
    await page.getByPlaceholder("Your email address").fill(`benchmark-${stamp}@ryvoko.test`);
...
async function launchDesktop(
  ...
  const app = await electron.launch({
    ...
    env: {
      ...benchmark.env,
      RYVOKO_PERFORMANCE_USER_DATA: profile,
      RYVOKO_PERFORMANCE_CLEAR_CACHE: clearCache ? "1" : "0",
    },
  });
...
async function measureInteractions(app: ElectronApplication, page: Page) {
  ...
  await page.evaluate(() => {
    ...
    (window as typeof window & { __ryvokoKeyPaintSamples?: number[] }).__ryvokoKeyPaintSamples =
      samples;
...
async function measureBundles() {
  ...
  const applications = await findNamed(path.join(desktopRoot, "out"), "Ryvoko.app");
...
function renderMarkdown(report: PerformanceReport) {
  ...
  return `# Ryvoko desktop performance — ${report.label}
