...
import type { ThreadSnapshot } from "@ryvoko/contracts";
...
describeAttachments("chat attachments", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-attachments-"));
  ...
  it("uploads, sends with image/file, and rejects invalid attachments", async () => {
    const cookie = await signup(app, `attachments-${stamp}@ryvoko.test`, "Attachment User");
  ...
  it("attaches a workspace file into the thread", async () => {
    const cookie = await signup(app, `attach-thread-${stamp}@ryvoko.test`, "Attach Thread User");
    const bot = await rpc<{ id: string }>(app, cookie, "bots/create", {
      name: "Attacher",
      title: "Attacher",
