...
describeLive("live release-watch eval (GPT 5.6 Luna + GitHub emulator)", () => {
  ...
  it("writes a concrete routine and reads seeded releases via GitHub tools, not the browser", async () => {
    ...
    dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-release-watch-"));
    ...
    const signup = await handles.app.request("/api/auth/sign-up/email", {
      ...
      body: JSON.stringify({
        email: `release-watch-${stamp}@ryvoko.test`,
        password: "password12",
        name: "Release Watch",
      }),
    });
    ...
    await rpc(handles.app, cookie, "threads/send", {
      botId: bot.id,
      text: [
        "Create a daily task to watch for new releases of Ryvexam/ryvoko",
    ...
    const routine =
      routines.find((row) => /ryvoko|release/i.test(`${row.name}\n${row.prompt}`)) ?? routines[0]!;
