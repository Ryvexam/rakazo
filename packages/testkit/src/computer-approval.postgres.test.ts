...
import { ComposioEmulator, FakeSandboxProvider } from "@ryvoko/adapters";
...
describe.skipIf(!databaseAvailable)("offline Pi computer approval", () => {
  beforeAll(() => {
    ...
    vi.stubEnv("RYVOKO_OPENAI_COMPATIBLE_VISION_MODELS", "offline-fixture");
  });
  ...
  it.each(["allow", "deny"] as const)(
    ...
    async (answer) => {
      ...
      const dataDir = await mkdtemp(path.join(tmpdir(), "ryvoko-computer-approval-"));
      ...
      try {
        ...
        const signup = await handles.app.request("/api/auth/sign-up/email", {
          ...
          body: JSON.stringify({
            email: `computer-approval-${randomUUID()}@ryvoko.test`,
            password: "password12",
