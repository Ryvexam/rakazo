...
import type { ComputerRef, SandboxProvider } from "@ryvoko/adapter-kit";
...
describeLive("real model and sandbox computer journey", () => {
  ...
  beforeAll(async () => {
    ...
    dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-computer-e2e-"));
  ...
  it("observes and clicks a real browser, then uses terminal and files", async () => {
    ...
    const signup = await handles.app.request("/api/auth/sign-up/email", {
      ...
      body: JSON.stringify({
        email: `computer-${stamp}@ryvoko.test`,
        password: "password12",
        name: "Computer E2E",
      }),
    });
...
PAGE = b'''<!doctype html><meta charset="utf-8"><title>Ryvoko visual test</title>
