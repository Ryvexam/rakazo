...
import { SCRIPTED_MPEG, SCRIPTED_TRANSCRIPT, SCRIPTED_VOICE_ID } from "@ryvoko/adapters";
...
describeVoice("voice credentials and speech HTTP", () => {
  ...
  const dataDir = mkdtempSync(path.join(tmpdir(), "ryvoko-voice-"));
  ...
  it("connects a scripted key, speaks, and transcribes without leaking the secret", async () => {
    const cookie = await signup(app, `voice-${stamp}@ryvoko.test`, "Voice User");
    const other = await signup(app, `voice-other-${stamp}@ryvoko.test`, "Other Voice");
