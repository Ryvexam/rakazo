...
function postgresUp() {
  try {
    execSync(`docker compose -f ${composeFile} exec -T postgres pg_isready -U ryvoko`, {
      stdio: "ignore",
      timeout: 8_000,
    });
    return true;
...
describeBackup("compose backup and restore", () => {
  it("dumps postgres and restores into a side database", () => {
    ...
    execSync(`${backupScript} ${stamp}`, {
      stdio: "pipe",
      timeout: 60_000,
      env: { ...process.env, RYVOKO_BACKUP_SKIP_HOMES: "1" },
    });
    const dump = path.resolve("backups", stamp, "ryvoko.sql");
    ...
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ryvoko -d postgres -c "DROP DATABASE IF EXISTS ryvoko_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ryvoko -d postgres -c "CREATE DATABASE ryvoko_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ryvoko -d ryvoko_restore_test`,
      {
        input: sql,
    ...
    const tables = execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ryvoko -d ryvoko_restore_test -c "\\dt"`,
      { encoding: "utf8", timeout: 20_000 },
    );
    expect(tables).toMatch(/bots/);
    execSync(
      `docker compose -f ${composeFile} exec -T postgres psql -U ryvoko -d postgres -c "DROP DATABASE ryvoko_restore_test"`,
      { stdio: "pipe", timeout: 20_000 },
    );
