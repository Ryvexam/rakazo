import type { TransactionalEmail, TransactionalEmailProvider } from "@ryvoko/adapter-kit";
import { emailAllowed, isMessagingEmail, parseAllowlist, signupPolicyFromEnv } from "@ryvoko/core";
import { bootstrapUserSpace, type PrismaClient } from "@ryvoko/db";
...
export function verificationEmail(email: string, url: string): TransactionalEmail {
  return {
    to: email,
    subject: "Verify your Ryvoko email",
    text: `Verify your email, then return to Ryvoko to sign in:\n\n${url}\n\nThis link expires in one hour. If you did not register, ignore this email.`,
    html: `<p><a href="${escapeHtml(url)}">Verify email</a>, then return to Ryvoko to sign in.</p><p>This link expires in one hour. If you did not register, ignore this email.</p>`,
  };
}
...
export function passwordResetEmail(
  ...
  return {
    to: user.email,
    subject: "Reset your Ryvoko password",
    text: [
      `Hi ${name},`,
      "",
      "Reset your Ryvoko password using this link:",
      resetUrl,
      "",
      "This link expires in one hour. If you did not request this, you can ignore this email.",
    ].join("\n"),
    html: `<p>Hi ${safeName},</p><p>Reset your Ryvoko password:</p><p><a href="${safeUrl}">Reset password</a></p><p>This link expires in one hour. If you did not request this, you can ignore this email.</p>`,
  };
}
