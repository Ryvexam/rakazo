...
import { avatarIdentitySeed } from "@ryvoko/core";
...
describe("BotAvatar", () => {
  ...
  it.each(["running", "queued", "leased", "waiting_input", "waiting_takeover"])(
    ...
    (status) => {
      ...
      expect(html).toContain("ryvoko-bot-avatar-ring");
    },
  );
  ...
  it("generates an organic avatar from the bot color", () => {
    ...
    expect(html).toContain("ryvoko-organic-avatar");
    expect(html).toContain('data-working="true"');
    expect(html).toMatch(/data-shape-family="\d"/);
    expect(html).toMatch(/data-eye-pattern="[0-3]"/);
    expect(html).toContain("<animate");
    expect(html).not.toContain("ryvoko-bot-avatar-visor");
  });
  ...
  it("keeps the organic morph timeline stable across status updates", () => {
    ...
    expect(idle).toContain("ryvoko-organic-avatar-eyes-idle");
    expect(idle).toContain("ryvoko-organic-avatar-eyes-working");
    expect(idle).toContain("ryvoko-organic-avatar-body-idle");
    expect(idle).toContain("ryvoko-organic-avatar-body-working");
