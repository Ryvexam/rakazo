...
export const GroupAvatar = memo(function GroupAvatar({
  ...
  if (!firstMember) {
    return (
      <div
        className={cn(
          "ryvoko-group-avatar relative flex items-center justify-center rounded-full border border-border bg-muted text-muted-foreground",
          className,
        )}
  ...
  if (members.length === 1) {
    return (
      <BotAvatar
        ...
        className={cn("ryvoko-group-avatar", className)}
      />
    );
  }
  ...
  return (
    <div
      className={cn("ryvoko-group-avatar relative rounded-full select-none", className)}
      style={{ width: size, height: size, flex: "none" }}
    >
