...
export const DEFAULT_DESKTOP_ENV: DesktopEnvironment = {
  homeDir: "/home/ryvoko",
  workspaceDir: "/home/ryvoko",
  browserProfilesDir: "/home/ryvoko/.browser-profiles",
  displayStart: 1,
...
function browserPidPathForScreen(screenId: string) {
  return `/tmp/ryvoko/browser-pid-${browserKeyForScreen(screenId)}`;
}
...
export function browserLauncherPath(displayNumber: number | string) {
  return `/tmp/ryvoko/browser-launch-${displayNumber}`;
}
...
function browserLauncherCommand(
  ...
  return [
    ...
    ...(index === undefined
      ...
      : []),
    "browser=$(command -v ryvoko-browser || command -v google-chrome || command -v google-chrome-stable || command -v chromium || command -v chromium-browser)",
...
export function stopAllDesktopBrowsersCommand(env = DEFAULT_DESKTOP_ENV) {
  const placeholder = "RYVOKO_INTERNAL_PROFILE";
  ...
  return [
    ...
    '  pid_file="/tmp/ryvoko/browser-pid-$hash"',
...
export function resetDesktopRuntimeCommand(env = DEFAULT_DESKTOP_ENV) {
  ...
  return [
    ...
    "for marker in /tmp/ryvoko/browser-profile-*; do",
    '  [ -f "$marker" ] || continue',
    // biome-ignore lint/suspicious/noTemplateCurlyInString: generated shell parameter expansion
    "  display=${marker##*-}",
    ...
    `rm -f ${TARGETS}/view-* ${TARGETS}/control-* /tmp/ryvoko/browser-pid-* /tmp/ryvoko/browser-profile-* /tmp/ryvoko/browser-launch-*`,
  ].join("\n");
}

const TARGETS = "/tmp/ryvoko/desktop-targets";
...
function revokeTargetCommand(kind: "view" | "control", display: number | string) {
  return `mkdir -p ${TARGETS}; : >/tmp/ryvoko/${kind}-target-next-${display}; mv /tmp/ryvoko/${kind}-target-next-${display} ${TARGETS}/${kind}-${display}`;
}

function stopVncCommand(kind: "view" | "control", layout: ReturnType<typeof commandLayout>) {
  const socketPrefix = `/tmp/ryvoko/sockets/${kind}-${layout.displayNumber}-`;
...
function gatewayCommand(port: string) {
  ...
  return [
    ...
    `exec 10>/tmp/ryvoko/gateway-${port}.lock; flock -w 120 10`,
    ...
    `  nohup "$proxy" --heartbeat=30 --web="$web" --token-plugin=TokenFile --token-source=${TARGETS} 0.0.0.0:${port} 8>&- 9>&- 10>&- </dev/null >/tmp/ryvoko/gateway-${port}.log 2>&1 &`,
    "fi",
...
function renderStopScreenTransportsCommand(index: number | undefined, env = DEFAULT_DESKTOP_ENV) {
  ...
  return [
    ...
    `rm -f /tmp/ryvoko/control-token-${layout.displayNumber}`,
  ].join("\n");
}
...
function renderStopExtraScreenCommand(
  ...
  return [
    ...
    `rm -f /tmp/ryvoko/browser-profile-${layout.displayNumber} ${browserLauncherPath(layout.displayNumber)}`,
...
function renderEnsureScreenCommand(
  ...
  const layout = commandLayout(index, env);
  const fluxHome = `/tmp/fluxbox-home-${layout.displayNumber}`;
  const log = `/tmp/ryvoko/screen-${layout.displayNumber}`;
  ...
  const setupDisplay =
    index === 0 && env.preservePrimaryDisplay
      ...
      : [
          ...
          `  mkdir -p /tmp/ryvoko ${fluxHome}/.fluxbox /tmp/.X11-unix`,
          `  rm -f /tmp/.X${layout.displayNumber}-lock /tmp/.X11-unix/X${layout.displayNumber}`,
          ...
          `  if [ -f /etc/ryvoko/fluxbox/init ]; then cp /etc/ryvoko/fluxbox/init ${fluxHome}/.fluxbox/init; else printf "session.screen0.toolbar.visible: false\\n" >${fluxHome}/.fluxbox/init; fi`,
          `  cp /etc/ryvoko/fluxbox/apps ${fluxHome}/.fluxbox/apps 2>/dev/null || true`,
  ...
  const socket = `/tmp/ryvoko/sockets/view-${layout.displayNumber}-\${desktop_view_token}`;
  const setupView = [
    `mkdir -p ${TARGETS} /tmp/ryvoko/sockets`,
    ...
    `[ -S ${socket} ] || exit 1`,
    `printf '%s: unix_socket:%s\\n' "$desktop_view_token" ${socket} >/tmp/ryvoko/view-target-next-${layout.displayNumber}`,
    `mv /tmp/ryvoko/view-target-next-${layout.displayNumber} ${targetFile}`,
  ...
  return [
    ...
    ...setupDisplay,
    `xdpyinfo -display ${layout.display} >/dev/null 2>&1 || exit 1`,
    `mkdir -p /tmp/ryvoko ${shellQuote(path.posix.dirname(profile))}`,
    ...browserRunningFunction(profile, pidFile),
    `printf %s ${shellQuote(profile)} >/tmp/ryvoko/browser-profile-${layout.displayNumber}`,
    "if ! browser_running; then",
...
export function interactiveScreenCommand(
  ...
  const tokenFile = `/tmp/ryvoko/control-token-${layout.displayNumber}`;
  ...
  if (!interactive) {
    ...
    return [
      ...
      stopProcesses,
      "printf 'RYVOKO_CONTROL_RELEASED\\n'",
      "fi",
    ].join("\n");
  }
  if (!controlToken) throw new Error("interactive screen requires a control token");
  const socket = `/tmp/ryvoko/sockets/control-${layout.displayNumber}-${browserKeyForScreen(controlToken)}`;
  return [
    ...
    `nohup x11vnc -display ${layout.display} -forever -shared -nopw -rfbport 0 -unixsock ${socket} -xkb -ncache 0 8>&- 9>&- </dev/null >/tmp/ryvoko/x11vnc-control-${layout.displayNumber}.log 2>&1 &`,
    `for i in $(seq 1 50); do [ -S ${socket} ] && break; sleep 0.1; done`,
    `[ -S ${socket} ] || exit 1`,
    `printf '%s: unix_socket:%s\\n' ${shellQuote(controlToken)} ${socket} >/tmp/ryvoko/control-target-next-${layout.displayNumber}`,
    `mv /tmp/ryvoko/control-target-next-${layout.displayNumber} ${targetFile}`,
    gatewayCommand(layout.controlPort),
  ].join("\n");
}

const REGISTRY = "/tmp/ryvoko/desktop-assignments";
const TOKEN_PLACEHOLDER = "RYVOKO_INTERNAL_VIEW_TOKEN";
...
for marker in pathlib.Path("/tmp/ryvoko").glob("browser-profile-*"):
...
export function releaseDesktopCommand(
  ...
  return [
    ...
    'rm -f "$slot"',
    'printf "RYVOKO_DESKTOP_RELEASED=%s\\n" "$index"',
  ].join("\n");
}
