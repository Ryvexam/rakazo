...
const RUNTIME_SECRETS_ERROR =
  "Set BETTER_AUTH_SECRET and ENCRYPTION_KEY to long random strings before starting Ryvoko outside local development or tests.";
...
export function isDevSecretAllowed(env: NodeJS.ProcessEnv = process.env): boolean {
  if (env.RYVOKO_ALLOW_DEV_SECRETS === "1") return true;
...
export function resolveUpdaterToken(env: NodeJS.ProcessEnv = process.env): string {
  return resolveDedicatedSecret(env, {
    name: "RYVOKO_UPDATER_TOKEN",
    conflicts: ["BETTER_AUTH_SECRET", "SANDBOX_SUPERVISOR_TOKEN", "SCREEN_PROXY_SECRET"],
    missingMessage: "Set RYVOKO_UPDATER_TOKEN to a dedicated random updater credential.",
    conflictMessage:
      "RYVOKO_UPDATER_TOKEN must differ from BETTER_AUTH_SECRET, SANDBOX_SUPERVISOR_TOKEN, and SCREEN_PROXY_SECRET.",
  });
}
 ...
 * through `@ryvoko/core`, and importing `node:crypto` here fails the production Vite build with
