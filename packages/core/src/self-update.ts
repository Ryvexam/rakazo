/** The repository a stock Ryvoko deployment tracks. */
export const OFFICIAL_REPO_URL = "https://github.com/Ryvexam/ryvoko";
...
/** `github.com/Ryvexam/ryvoko` for every spelling of the same remote, for comparison only. */
...
export function updateSteps(input: UpdatePlanInput): UpdateStep[] {
  ...
  steps.push(
    ...
    },
    {
      ...
      command: "pnpm",
      args: ["--filter", "@ryvoko/db", "run", "generate"],
    },
    {
      id: "build",
      label: "Build the web app",
      command: "pnpm",
      args: ["--filter", "@ryvoko/web", "run", "build"],
    },
    {
      id: "migrate",
      label: "Apply database migrations",
      command: "pnpm",
      args: ["--filter", "@ryvoko/db", "run", "migrate"],
    },
  );
  return steps;
}
...
export const RESTART_SUPERVISOR_ENV = "RYVOKO_UPDATE_RESTART_SUPERVISOR";
...
export function restartSupervisorAdvice(supervisor: RestartSupervisor): string {
  if (supervisor.kind !== "none") {
    return `Ryvoko will exit after updating and ${supervisor.label} will start it on the new code.`;
  }
  return `No process supervisor was detected, so Ryvoko will not exit on its own. Restart the API, worker, and web processes yourself, or set ${RESTART_SUPERVISOR_ENV} to the supervisor that restarts them (for example "docker" with restart: unless-stopped, or run under systemd with Restart=always).`;
}
