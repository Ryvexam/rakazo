...
afterEach(() => {
  installLogger(createLogger({ service: "ryvoko", level: "off", sinks: [] }));
});
...
describe("job correlation envelope", () => {
  ...
  it("uses a nested envelope for reserved keys and non-plain objects", () => {
    ...
    const reserved = wrapJobPayload({ runId: "run-2", __ryvokoLog: "keep" }, correlation);
    expect(reserved).toEqual({
      v: 1,
      correlation,
      payload: { runId: "run-2", __ryvokoLog: "keep" },
    });
