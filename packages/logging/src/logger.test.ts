...
describe("logger", () => {
  it("filters levels including off", () => {
    ...
    const logger = createLogger({ service: "ryvoko-api", level: "warn", sinks: [sink] });
    logger.debug("d");
    logger.info("i");
    logger.warn("w");
    logger.error("e");
    ...
    const silent = createLogger({ service: "ryvoko-api", level: "off", sinks: [sink] });
    silent.error("never");
    expect(sink.events.map((event) => event.message)).toEqual(["w", "e"]);
  });
  ...
  it("keeps the configured service name when bindings try to overwrite it", () => {
    const sink = createTestSink();
    const logger = createLogger({ service: "ryvoko-api", sinks: [sink] });
    logger.info("svc", { "service.name": "spoofed" });
    expect(sink.events[0]?.["service.name"]).toBe("ryvoko-api");
  });
