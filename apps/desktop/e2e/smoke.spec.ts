...
import type { RyvokoDesktop } from "@ryvoko/contracts";

const fixture = `<!doctype html>
<html lang="en">
  <head><meta charset="utf-8"><title>Ryvoko desktop smoke</title></head>
...
test("launches with a narrow preload bridge and an isolated renderer", async () => {
  const app = await electron.launch({
    ...
    env: {
      ...process.env,
      RYVOKO_WEB_URL: `data:text/html;charset=utf-8,${encodeURIComponent(fixture)}`,
    },
  });
  ...
  try {
    ...
    await expect(page).toHaveTitle("Ryvoko desktop smoke");

    const renderer = await page.evaluate(async () => {
      const desktop = (window as typeof window & { ryvokoDesktop?: RyvokoDesktop }).ryvokoDesktop;
