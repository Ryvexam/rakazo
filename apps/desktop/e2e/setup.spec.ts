...
const APP_MARKER = "Existing Ryvoko instance ready";
...
test.beforeAll(async () => {
  server = createServer((request, response) => {
    ...
    response.end(
      `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ryvoko</title></head><body><main>${APP_MARKER}</main></body></html>`,
    );
  });
...
test.beforeEach(async () => {
  userData = await mkdtemp(path.join(tmpdir(), "ryvoko-desktop-e2e-"));
});
...
function launch(extraEnv: Record<string, string> = {}) {
  const env = { ...process.env, RYVOKO_PERFORMANCE_USER_DATA: userData };
  // A stale RYVOKO_WEB_URL from the developer's shell would bypass setup entirely.
  delete env.RYVOKO_WEB_URL;
  const executablePath = process.env.RYVOKO_E2E_EXECUTABLE;
...
test("first run asks whether to use a local or existing instance", async () => {
  app = await launch();
  if (process.env.RYVOKO_E2E_EXECUTABLE) {
    expect(await app.evaluate(({ app }) => app.isPackaged)).toBe(true);
  }
  const setup = await app.firstWindow();

  await expect(setup.getByRole("heading", { name: "Welcome to Ryvoko" })).toBeVisible();
...
test("connecting to an existing instance verifies, saves, and opens it", async () => {
  ...
  await expect(setup.locator("#status")).toHaveText(`Ryvoko answered at ${serverUrl}.`);
...
test("a session-pending shell skeleton is not accepted as a ready app", async () => {
  const skeletonHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ryvoko</title></head>
<body><div id="root"><div data-ryvoko-app-state="session-pending"><aside></aside><main><div>Opening your Space…</div></main></div></div></body></html>`;
...
for (const { name, surface } of [
  ...
  },
  {
    ...
    surface: '<div data-ryvoko-surface="welcome"><button>Sign up</button></div>',
  },
  {
    name: "translated logged-out welcome",
    surface: '<div data-ryvoko-surface="welcome"><button>Créer un compte</button></div>',
  },
]) {
  test(`a post-session ${name} mount is accepted`, async () => {
    const readyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ryvoko</title>
</head>
<body><div id="root"><div data-ryvoko-app-state="ready">${surface}</div></div></body></html>`;
    ...
    try {
      ...
      await expect(appWindow.locator('[data-ryvoko-app-state="ready"]')).toBeVisible();
      await expect
        .poll(async () => {
          try {
            return JSON.parse(await readFile(path.join(userData, "setup.json"), "utf8"));
...
test("a shell mount before workspace bootstrap is not accepted", async () => {
  const preBootstrapHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ryvoko</title></head>
<body><div id="root"><div data-ryvoko-app-state="ready"><div data-testid="shell-root" data-ready="false">Workspace</div></div></div></body></html>`;
...
test("a session-ready marker without a route surface is not accepted", async () => {
  const emptyReadyHtml = `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>Ryvoko</title></head>
<body><div id="root"><div data-ryvoko-app-state="ready" class="h-full"></div></div></body></html>`;
...
test("a generic web page is not accepted as a Ryvoko server", async () => {
  const plain = createServer((_request, response) => {
    response.writeHead(200, { "content-type": "text/html; charset=utf-8" });
    response.end("<!doctype html><p>not Ryvoko</p>");
  });
  ...
  try {
    ...
    await expect(setup.locator("#status")).toHaveText(
      "That address did not respond like a Ryvoko server.",
    );
    await expect(async () => {
      await expect(readFile(path.join(userData, "setup.json"), "utf8")).rejects.toThrow();
    }).toPass();
...
test("the native application menu can reopen setup without exposing setup IPC to the server", async () => {
  app = await launch({ RYVOKO_WEB_URL: serverUrl });
  const appWindow = await app.firstWindow();
  await expect(appWindow.getByText(APP_MARKER)).toBeVisible();

  const setupPromise = app.waitForEvent("window");
  await app.evaluate(({ Menu }) => {
    const item = Menu.getApplicationMenu()?.getMenuItemById("change-ryvoko-server");
...
test("servers on the same host but different ports do not share login cookies", async () => {
  const first = createServer((_request, response) => {
    response.writeHead(200, {
      ...
      "set-cookie": "ryvoko_session=fake-one; Path=/; SameSite=Lax",
    });
  ...
  try {
    app = await launch({ RYVOKO_WEB_URL: `http://127.0.0.1:${firstAddress.port}` });
    ...
    app = await launch({ RYVOKO_WEB_URL: `http://127.0.0.1:${secondAddress.port}` });
...
test("setup IPC is not reachable from the connected app window", async () => {
  ...
  const exposed = await appWindow.evaluate(() =>
    Object.keys((window as typeof window & { ryvokoSetup?: unknown }).ryvokoSetup ?? {}),
  );
