...
const APP_MARKER = "Local Ryvoko stack ready";
const IMAGE_TAG = "v9.9.9";
const STACK_PROBE_PATH = "/.well-known/ryvoko-desktop-stack";
const STACK_TOKEN_HEADER = "x-ryvoko-desktop-stack-token";
...
test.beforeAll(async () => {
  server = createServer(async (request, response) => {
    ...
    if (request.url === "/api/desktop-settings/rpc/integrationSetup/get") {
      const expected = await readFile(path.join(userData, "stack", ".desktop-stack-token"), "utf8");
      if (
        request.headers["x-ryvoko-local-settings-token"] !== expected.trim() ||
...
test.beforeEach(async () => {
  ...
  userData = await realpath(await mkdtemp(path.join(tmpdir(), "ryvoko-desktop-stack-")));
});
...
/**
 * Stands in for the docker CLI: records `cwd | RYVOKO_IMAGE_TAG | argv` for every call and
...
async function writeFakeDocker(mode: FakeDockerMode) {
  ...
  let up = '  up) echo "Container ryvoko-web-1 Started"; exit 0 ;;';
  if (mode === "pool-exhausted") {
    ...
    up = `  up) if [ ! -f '${path.join(userData, "port-conflict")}' ]; then touch '${path.join(userData, "port-conflict")}'; echo "port is already allocated" >&2; exit 1; fi; echo "Container ryvoko-web-1 Started"; exit 0 ;;`;
  }
  const lines = [
    "#!/bin/sh",
    `printf '%s | %s | %s\\n' "$PWD" "$RYVOKO_IMAGE_TAG" "$*" >> '${fakeDockerLog()}'`,
    'case "$1 $2" in',
    ...
    mode === "daemon-down"
      ...
      : '    echo "27.1.1"; exit 0 ;;',
    "esac",
    "# compose --env-file .env -f docker-compose.images.yml --project-name ryvoko-desktop <command> ...",
    'case "$8" in',
    "  pull)",
    mode === "pull-fails"
...
async function launch(mode: FakeDockerMode | "missing") {
  const env: NodeJS.ProcessEnv = { ...process.env, RYVOKO_PERFORMANCE_USER_DATA: userData };
  ...
  return electron.launch({
    ...
    env: {
      ...env,
      RYVOKO_DOCKER_BINARY:
        mode === "missing" ? "/nonexistent/docker" : await writeFakeDocker(mode),
      RYVOKO_LOCAL_WEB_URL: serverUrl,
      RYVOKO_IMAGE_TAG: IMAGE_TAG,
    },
  });
}
...
test("This computer installs and starts the stack, then opens the app", async () => {
  ...
  await expect(setup.locator("#stack-phase")).toHaveText("Downloading Ryvoko…");
  ...
  expect(env).not.toContain("RYVOKO_IMAGE_TAG=");
  expect(env).not.toContain("RYVOKO_COMPUTER_IMAGE_TAG=");
  ...
  const compose =
    "compose --env-file .env -f docker-compose.images.yml --project-name ryvoko-desktop";
...
test("the native settings menu opens an isolated logged-out settings capability", async () => {
  ...
  const denied = await main.evaluate(async () => {
    try {
      await window.ryvokoDesktop?.localSettings?.request(
  ...
  const result = await settings.evaluate(() =>
    window.ryvokoDesktop?.localSettings?.request(
      "/api/desktop-settings/rpc/integrationSetup/get",
      "{}",
    ),
  );
