...
import type { RyvokoDesktopOAuthCallback } from "@ryvoko/contracts";
...
export async function openBrowserAuth(
  ...
  options: {
    ...
    onCallback: (callback: RyvokoDesktopOAuthCallback) => void;
    onClose?: () => void;
  },
): Promise<void> {
      ...
      for (const host of hosts) {
        ...
        const server = createServer({ maxHeaderSize: 8192 }, (request, response) => {
          ...
          if (
            ...
            response.writeHead(400).end("Sign-in callback rejected. Return to Ryvoko to retry.");
            return;
          }
          consumed = true;
          response.end("You can close this tab and return to Ryvoko.", close);
