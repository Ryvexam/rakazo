...
import type { DesktopSetup, DesktopStackProbeResponse } from "@ryvoko/contracts";
...
/**
 * Accepts what a person would actually type ("localhost:5173", "ryvoko.example.com")
 * and returns a canonical http(s) origin, or null when the input can never
 * securely address a Ryvoko server.
 */
export function normalizeServerUrl(input: string): string | null {
  ...
  // Ryvoko serves its renderer, RPC, and auth routes from one origin. Keeping a
 ...
 * `RYVOKO_WEB_URL` still wins over saved configuration so test and performance
...
/** Each Ryvoko origin gets its own persistent cookie and storage partition. */
...
export function isRyvokoHealth(value: unknown): boolean {
 ...
 * metadata endpoints. Cleartext HTTP to them is never a legitimate Ryvoko
