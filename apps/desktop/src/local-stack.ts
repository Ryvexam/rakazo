...
import type { DesktopLocalStackState } from "@ryvoko/contracts";
import {
...
export const STACK_PROJECT_NAME = "ryvoko-desktop";
...
const LAUNCH_SUPPLIED = ["RYVOKO_IMAGE_TAG", "RYVOKO_COMPUTER_IMAGE_TAG"];
  ...
  switch (kind) {
    ...
    case "other":
      return phase === "pulling"
        ? "Downloading Ryvoko images failed. Check the output below, then retry."
        : "Ryvoko services did not start. Check the output below, then retry.";
  }
}
  ...
  private docker(binary: string, args: string[], timeoutMs: number, signal?: AbortSignal) {
    return this.deps.run(binary, args, {
      ...
      env: dockerSpawnEnv(this.deps.platform, this.deps.env, binary, {
        RYVOKO_IMAGE_TAG: this.deps.imageTag,
        RYVOKO_COMPUTER_IMAGE_TAG: this.deps.imageTag,
        ...(this.currentStackToken === null
          ? {}
          : { RYVOKO_DESKTOP_STACK_TOKEN: this.currentStackToken }),
        RYVOKO_WEB_PORT: new URL(this.currentWebUrl).port || "80",
