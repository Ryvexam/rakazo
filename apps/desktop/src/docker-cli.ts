...
import type { DesktopSetupLink } from "@ryvoko/contracts";
...
/**
 * `RYVOKO_DOCKER_BINARY` is a test hook and the only candidate when set, so a
...
export function resolveDockerBinary(
  ...
  const override = env.RYVOKO_DOCKER_BINARY?.trim();
