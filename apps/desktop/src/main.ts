...
import type { DesktopReachability, DesktopSetup } from "@ryvoko/contracts";
import { LOCAL_SETTINGS_PAGE } from "@ryvoko/contracts/local-settings";
import {
  app,
  BrowserWindow,
  dialog,
...
const PERFORMANCE_USER_DATA = process.env.RYVOKO_PERFORMANCE_USER_DATA;
/** Test hook: where the app-managed stack answers. Mode `new` still requires loopback. */
const LOCAL_WEB_URL = process.env.RYVOKO_LOCAL_WEB_URL?.trim() || DEFAULT_LOCAL_WEB_URL;
const PROBE_TIMEOUT_MS = 8_000;
const DESKTOP_STACK_PROBE_PATH = "/.well-known/ryvoko-desktop-stack";
const DESKTOP_STACK_TOKEN_HEADER = "x-ryvoko-desktop-stack-token";
...
const WARM_WINDOW_TTL_MS = warmWindowTtlMs(process.env.RYVOKO_WARM_WINDOW_TTL_MS);

const updaterEnvironment = {
  packaged: app.isPackaged,
  version: app.getVersion(),
  disabled: process.env.RYVOKO_DISABLE_AUTO_UPDATE === "1",
};
...
function createWindow(url: string, partition: string | null) {
  ...
  win.on("close", (event) => {
    if (
      ...
      !quitting &&
      process.env.RYVOKO_DISABLE_WARM_WINDOW !== "1"
    ) {
      event.preventDefault();
      win.hide();
 ...
 * fixtures omit the Ryvoko app-state marker.
 */
async function waitForMountedAppDocument(contents: Electron.WebContents) {
  ...
  while (Date.now() < deadline) {
    ...
    const ready = (await contents.executeJavaScript(`(() => {
      const appState =
        document.querySelector("[data-ryvoko-app-state]")?.getAttribute("data-ryvoko-app-state") ??
        null;
      ...
      const authOrWelcomeSurface = Boolean(
        document.querySelector('[data-ryvoko-surface="welcome"]') ||
      ...
      // Desktop e2e fixtures mount a plain page without Ryvoko app-state markers.
      if (appState === null) {
...
async function installBundledRenderer(
  ...
  if (!app.isPackaged || process.env.RYVOKO_DISABLE_BUNDLED_RENDERER === "1") return;
...
function installApplicationMenu() {
  ...
  const changeServer: Electron.MenuItemConstructorOptions = {
    id: "change-ryvoko-server",
    label: "Change Ryvoko Server…",
...
/** Setup IPC must only answer the setup window, never a connected Ryvoko server. */
  ...
  try {
    ...
    if (response.status >= 300 && response.status < 400) {
      return {
        ...
        url,
        error: "That address redirects elsewhere. Enter the final Ryvoko server address.",
      };
    }
    if (!response.ok) {
      return {
    ...
    if (!isRyvokoHealth(health)) {
      return {
        ok: false,
        status: response.status,
        url,
        error: "That address did not respond like a Ryvoko server.",
      };
    }
    return {
      ok: true,
...
app.whenReady().then(async () => {
  ...
  const target = resolveStartupTarget({
    envUrl: process.env.RYVOKO_WEB_URL,
    saved: currentSetup,
    forceSetup: process.env.RYVOKO_FORCE_SETUP === "1",
  });
  if (process.env.RYVOKO_PERFORMANCE_CLEAR_CACHE === "1") {
    ...
    try {
      ...
      if (setup.mode === "new") {
        ...
        if (managedUrl === null || !(await localStack.matchesDesiredStack())) {
          return {
            ok: false,
            error: "The app-managed Ryvoko services are not ready. Retry setup.",
          };
        }
