...
import type { RyvokoDesktop, RyvokoSetup } from "@ryvoko/contracts";
...
describe("desktop preload bridge", () => {
  it("exposes the scoped desktop bridges", async () => {
    ...
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, RyvokoDesktop];
    expect(globalName).toBe("ryvokoDesktop");
  ...
  it("forwards captured codes without leaking the IPC event to the renderer", () => {
    ...
    const [, bridge] = exposeInMainWorld.mock.calls[0] as [string, RyvokoDesktop];
...
describe("setup preload bridge", () => {
  it("exposes only the first-run setup operations", async () => {
    ...
    expect(exposeInMainWorld).toHaveBeenCalledTimes(1);
    const [globalName, bridge] = exposeInMainWorld.mock.calls[0] as [string, RyvokoSetup];
    expect(globalName).toBe("ryvokoSetup");
    expect(bridge.platform).toBe("linux");
    expect(Object.keys(bridge).sort()).toEqual([
      "openLink",
      "platform",
