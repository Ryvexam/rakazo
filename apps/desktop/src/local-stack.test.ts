...
describe("stack locations", () => {
  it("keeps the compose project under user data", () => {
    expect(stackDir("/data/ryvoko")).toBe(path.join("/data/ryvoko", "stack"));
  });
...
describe("resolveImageTag", () => {
  ...
  it("lets RYVOKO_IMAGE_TAG override everything", () => {
...
describe("renderStackEnv", () => {
  it("fills the five secrets and drops the image tag lines of the shipped template", async () => {
    ...
    expect(lines.some((line) => line.startsWith("RYVOKO_IMAGE_TAG="))).toBe(false);
    expect(lines.some((line) => line.startsWith("RYVOKO_COMPUTER_IMAGE_TAG="))).toBe(false);
    // Everything else, including the image names and empty optional keys, stays verbatim.
    expect(lines).toContain("RYVOKO_IMAGE=ghcr.io/ryvexam/ryvoko/app");
...
describe("ensureStackEnv", () => {
  let dir: string;
  beforeEach(async () => {
    dir = await mkdtemp(path.join(tmpdir(), "ryvoko-stack-env-"));
  });
  afterEach(async () => {
...
describe("stack identity", () => {
  let dir: string;
  beforeEach(async () => {
    dir = await mkdtemp(path.join(tmpdir(), "ryvoko-stack-token-"));
  });
  afterEach(async () => {
    await rm(dir, { recursive: true, force: true });
  });
...
const ok: Script = (args) => {
  ...
  if (subcommand === "up") return { lines: ["Container ryvoko-web-1 Started"] };
  return {};
};
...
describe("LocalStackController", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(path.join(tmpdir(), "ryvoko-stack-"));
    calls = [];
    phases = [];
  });
  afterEach(async () => {
  ...
  it("installs the compose project and walks every phase to ready", async () => {
    ...
    expect(state.output).toEqual([
      "app Pulled",
      "computer Pulled",
      "Container ryvoko-web-1 Started",
    ]);
    ...
    for (const call of calls) {
      ...
      expect(call.env).toMatchObject({
        RYVOKO_IMAGE_TAG: "v1.2.3",
        RYVOKO_COMPUTER_IMAGE_TAG: "v1.2.3",
        COMPOSE_PROGRESS: "plain",
        HOME: "/home/me",
      });
      expect(call.env).not.toHaveProperty("OPENROUTER_API_KEY");
    }
    expect(calls.at(-1)?.env.RYVOKO_DESKTOP_STACK_TOKEN).toBe("ab".repeat(32));
  ...
  it("keeps lifecycle commands off the standalone project despite environment overrides", async () => {
    ...
    await writeFile(path.join(dir, STACK_ENV_FILE), "COMPOSE_PROJECT_NAME=ryvoko\n");
    const stack = controller({ env: { COMPOSE_PROJECT_NAME: "ryvoko" } }, (args) =>
      args[7] === "up" ? { code: 1, stderr: "port is already allocated" } : ok(args),
    );
    expect((await stack.start()).phase).toBe("failed");
    // A fresh process must stop the same desktop project without discovering or adopting ryvoko.
    ...
    for (const call of commands) {
      expect(call.args.slice(5, 7)).toEqual(["--project-name", "ryvoko-desktop"]);
  ...
  it("retries port conflicts on a new origin and uses it for auth, health, and saved launches", async () => {
    ...
    expect(
      calls.filter((call) => call.args[7] === "up").map((call) => call.env.RYVOKO_WEB_PORT),
    ).toEqual(["5173", "45174"]);
      ...
      const stack = controller({}, (args) => {
        if (args[7] === "up" && exhausted) {
          return {
            code: 1,
            [stream]:
              "failed to create network ryvoko-desktop_data: Error response from daemon: all predefined address pools have been fully subnetted",
          };
        }
        return ok(args);
      });
