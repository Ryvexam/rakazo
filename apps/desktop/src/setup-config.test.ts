...
describe("server address normalization", () => {
  it("assumes http locally and https for a bare public host", () => {
    ...
    expect(normalizeServerUrl("ryvoko.example.com")).toBe("https://ryvoko.example.com");
  });

  it("keeps an explicit secure scheme and port but stores only the origin", () => {
    expect(normalizeServerUrl("https://ryvoko.example.com")).toBe("https://ryvoko.example.com");
    expect(normalizeServerUrl("https://ryvoko.example.com:8443/team")).toBe(
      "https://ryvoko.example.com:8443",
    );
  });
  ...
  it("rejects cleartext public servers but permits private-network development", () => {
    expect(normalizeServerUrl("http://ryvoko.example.com")).toBeNull();
  ...
  it("rejects cleartext link-local addresses used by cloud metadata endpoints", () => {
    ...
    // HTTPS to link-local still normalizes; the health probe must match Ryvoko.
  ...
  it.each(["", "   ", "not a url", "ftp://example.com", "file:///etc/passwd", "http://"])(
    "rejects an address that cannot reach a Ryvoko server (%s)",
    (value) => {
  ...
  it("rejects embedded credentials rather than writing them to disk", () => {
    expect(normalizeServerUrl("https://user:secret@ryvoko.example.com")).toBeNull();
  });
});
...
describe("managed local open URL", () => {
  ...
  it("rejects a different loopback origin even when both are local", () => {
    ...
    expect(managedLocalOpenUrl("https://ryvoko.example.com", DEFAULT_LOCAL_WEB_URL)).toBeNull();
  });
});

describe("desktop stack token transport", () => {
  it("allows HTTPS and loopback HTTP only", () => {
    expect(maySendDesktopStackToken("https://ryvoko.example.com")).toBe(true);
    ...
    expect(maySendDesktopStackToken("http://ryvoko.local:5173")).toBe(false);
  });
});

describe("saved setup", () => {
  it("round-trips through the on-disk format", () => {
    const setup = { mode: "existing", serverUrl: "https://ryvoko.example.com" } as const;
...
describe("startup target", () => {
  const saved = { mode: "existing", serverUrl: "https://ryvoko.example.com" } as const;
  ...
  it("opens the saved instance on later launches", () => {
    expect(resolveStartupTarget({ saved })).toEqual({
      kind: "app",
      url: "https://ryvoko.example.com",
      source: "saved",
    });
  });

  it("lets RYVOKO_WEB_URL point the shell anywhere without touching saved setup", () => {
  ...
  it("ignores an empty RYVOKO_WEB_URL", () => {
...
describe("bundled renderer eligibility", () => {
  it("stands in for http(s) origins only", () => {
    ...
    expect(servesBundledRenderer("https://ryvoko.example.com")).toBe(true);
...
describe("remote-content isolation", () => {
  it("uses a stable, opaque session partition per server origin", () => {
    ...
    expect(first).toMatch(/^persist:ryvoko-[a-f0-9]{24}$/);
...
describe("Ryvoko health response", () => {
  it("requires the public RPC health contract", () => {
    expect(isRyvokoHealth({ json: { ok: true, version: "0.1.0" } })).toBe(true);
    expect(isRyvokoHealth({ json: { ok: true } })).toBe(false);
    expect(isRyvokoHealth({ ok: true, version: "0.1.0" })).toBe(false);
  });
});
