(() => {
  const bridge = window.ryvokoSetup;
  ...
  const PHASE_LABELS = {
    ...
    pulling: "Downloading Ryvoko…",
    starting: "Starting Ryvoko…",
    "waiting-healthy": "Almost ready…",
    ready: "Ryvoko is ready.",
  };
  ...
  async function check() {
    ...
    try {
      ...
      if (result.ok) {
        ...
        setStatus(`Ryvoko answered at ${result.url}.`, "ok");
      } else {
  ...
  async function init() {
    ...
    try {
      ...
      setStatus("Setup could not start. Quit Ryvoko and try again.", "error");
      setBusy(true);
    }
  }
