...
const base = {
  DATABASE_URL: "postgres://ryvoko:ryvoko@127.0.0.1:5433/ryvoko",
  NODE_ENV: "test",
};
...
describe("loadEnv", () => {
  ...
  it("exposes a deployed git revision when GIT_SHA is set", () => {
    ...
    expect(loadEnv({ ...base, RYVOKO_GIT_SHA: "abc1234" }).gitSha).toBe("abc1234");
  });
  ...
  it("loads optional updater sidecar wiring without requiring the token at boot", () => {
    ...
    const env = loadEnv({
      ...base,
      RYVOKO_UPDATER_URL: " http://updater:7092 ",
  ...
  it("loads SMTP configuration and keeps the email emulator out of production", () => {
    expect(
      loadEnv({
        ...
        EMAIL_FROM: " Ryvoko <no-reply@example.test> ",
        EMAIL_EMULATOR: "true",
      }),
    ).toMatchObject({
      smtpUrl: "smtps://user:secret@smtp.example.test:465",
      emailFrom: "Ryvoko <no-reply@example.test>",
      emailEmulator: true,
    });
    expect(
      loadEnv({
