...
import { runContinueJob } from "@ryvoko/adapter-kit";
import { AutomatedSenderPoliciesSchema, type MessageBlock } from "@ryvoko/contracts";
import { BOT_MESSAGE_MAX_HOPS } from "@ryvoko/core";
...
export class TeamChatBridge {
  ...
  async receive(
    ...
    if (
      ...
      throw new Error("Team chat conversation belongs to a different Ryvoko target");
    }
    const now = new Date();
  ...
  private async ensureTranscriptMessage(
    ...
    if (!conversation.thread) throw new Error("Team chat conversation has no Ryvoko thread");
  ...
  private async deliverDelegatedReplies(target: TargetBot): Promise<void> {
    ...
    for (const run of runs) {
      ...
      const content =
        run.status === "failed"
          ? `${target.name} could not complete the delegated request. Open Ryvoko for details.`
  ...
  private async queue(message: {
    ...
    if (!thread) throw new Error("Team chat conversation has no Ryvoko thread");
  ...
  private async deliverFailure(message: {
    ...
    await this.sendOnce(message.id, {
      conversationId: message.externalConversation.conversationId,
      replyThreadId: message.replyThreadId,
      content: `${this.target?.name ?? "The agent"} could not complete that request. Open Ryvoko for details.`,
