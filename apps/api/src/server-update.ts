...
import { ServerUpdateRunSchema } from "@ryvoko/contracts";
import {
...
export interface UpdaterProxyConfig {
  ...
  /** Current `RYVOKO_IMAGE_TAG` when known; selects compose pull vs rebuild commands. */
...
export async function readServerUpdateStatus(
  ...
  const imageTagHint = config.imageTag?.trim() || process.env.RYVOKO_IMAGE_TAG?.trim() || null;
