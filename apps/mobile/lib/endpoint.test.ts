...
describe("normalizeApiBase", () => {
  it("trims, adds https, and keeps only the origin", () => {
    ...
    expect(normalizeApiBase("https://ryvoko.example.com:8443/api/")).toEqual({
      ok: true,
      url: "https://ryvoko.example.com:8443",
    });
...
describe("display and warnings", () => {
  ...
  it("shows host and non-default port", () => {
    expect(displayApiHost("https://ryvoko.example.com")).toBe("ryvoko.example.com");
  ...
  it("treats the compile-time default as not custom", () => {
    ...
    expect(usesCustomApiBase("https://ryvoko.example.com")).toBe(true);
  });
});

describe("probeApiBase", () => {
  it("accepts a Ryvoko /rpc/health response", async () => {
  ...
  it("rejects a host that is up but is not Ryvoko", async () => {
    const fetchImpl = vi.fn(
