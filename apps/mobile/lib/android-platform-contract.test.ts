...
describe("Android mobile platform contract", () => {
  ...
  it("requests live-update promotion and exposes its Android settings", () => {
    const nativeRoot = resolve(
      mobileRoot,
      "modules/ryvoko-notifications/android/src/main/java/com/ryvoko/notifications",
    );
    const service = readFileSync(resolve(nativeRoot, "RyvokoNotificationService.kt"), "utf8");
    const module = readFileSync(resolve(nativeRoot, "RyvokoNotificationsModule.kt"), "utf8");
    ...
    expect(module).toContain("RyvokoNotificationService.clearSession(context)");
    ...
    expect(service).toContain('connection.setRequestProperty("x-ryvoko-space-id", spaceId)');
    ...
    expect(service).toContain('"ryvoko:///group-thread?groupId=');
    expect(service).toMatch(/&spaceId=\$\{Uri\.encode\(run\.spaceId\)\}/);
    expect(service).toContain('putString("ryvoko.spaceId", run.spaceId)');
  ...
  it("shows live updates only for working runs and ties the pill to a real bot", () => {
    const service = readFileSync(
      resolve(
        mobileRoot,
        "modules/ryvoko-notifications/android/src/main/java/com/ryvoko/notifications/RyvokoNotificationService.kt",
      ),
      "utf8",
    );
    expect(service).toContain(
    ...
    expect(service).toContain('putString("ryvoko.botId", run.botId)');
