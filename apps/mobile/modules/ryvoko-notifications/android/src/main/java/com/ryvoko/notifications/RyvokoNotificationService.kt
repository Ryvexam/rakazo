package com.ryvoko.notifications
...
class RyvokoNotificationService : Service() {
  ...
  private fun post(run: RunRecord, copy: NotificationCopy) {
    ...
    val notification = builder(copy.channel)
      .setSmallIcon(R.drawable.ic_ryvoko_notification)
      .setContentTitle(copy.title)
      .setContentText(copy.body)
      .setStyle(Notification.BigTextStyle().bigText(copy.body))
      .setContentIntent(openApp(run))
      .addExtras(Bundle().apply {
        putString("ryvoko.spaceId", run.spaceId)
        putString("ryvoko.botId", run.botId)
        putString("ryvoko.threadId", run.threadId)
      })
      .setAutoCancel(true)
  ...
  private fun liveStatusIcon(run: RunRecord, avatarStyle: String): Icon {
    if (avatarStyle != "organic") {
      return Icon.createWithResource(this, R.drawable.ic_ryvoko_notification)
    }
  ...
  private fun openApp(run: RunRecord? = null): PendingIntent {
    val intent = if (run == null) {
      packageManager.getLaunchIntentForPackage(packageName) ?: Intent(Intent.ACTION_VIEW, Uri.parse("ryvoko://"))
    } else {
      val destination = if (run.groupId != null) {
        "ryvoko:///group-thread?groupId=${Uri.encode(run.groupId)}&name=${Uri.encode(run.groupName.orEmpty())}&spaceId=${Uri.encode(run.spaceId)}"
      } else {
        "ryvoko:///thread?botId=${Uri.encode(run.botId)}&name=${Uri.encode(run.botName)}&spaceId=${Uri.encode(run.spaceId)}"
      }
      Intent(
        Intent.ACTION_VIEW,
  ...
  companion object {
    ...
    private const val ACTION_THREAD_CHANGED = "com.ryvoko.notifications.THREAD_CHANGED"
    private const val PROMOTED_ONGOING_EXTRA = "android.requestPromotedOngoing"
    private const val POLL_INTERVAL_MS = 8_000L
    private const val STATE_PREFERENCES = "com.ryvoko.notification_state"
    ...
    fun start(context: Context) {
      val intent = Intent(context, RyvokoNotificationService::class.java)
      context.startService(intent)
    }

    fun stop(context: Context) {
      context.stopService(Intent(context, RyvokoNotificationService::class.java))
    }
    ...
    fun setOpenThread(context: Context, botId: String?, threadId: String?) {
      ...
      context.startService(
        Intent(context, RyvokoNotificationService::class.java).setAction(ACTION_THREAD_CHANGED),
      )
    }
...
private object Channels {
  const val LIVE = "ryvoko_live"
  const val MESSAGES = "ryvoko_messages"
  const val SCHEDULED = "ryvoko_scheduled"
  const val ATTENTION = "ryvoko_attention"
}
...
private fun rpc(
  ...
  return try {
    ...
    connection.setRequestProperty("Origin", "ryvoko://")
    connection.setRequestProperty("Authorization", "Bearer $token")
    if (spaceId.isNotBlank()) {
      connection.setRequestProperty("x-ryvoko-space-id", spaceId)
    }
