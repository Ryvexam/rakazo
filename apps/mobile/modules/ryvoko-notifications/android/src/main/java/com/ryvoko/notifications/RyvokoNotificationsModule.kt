...
class RyvokoNotificationsModule : Module() {
  override fun definition() = ModuleDefinition {
    Name("RyvokoNotifications")
    ...
    AsyncFunction("setSettings") { value: Map<String, Boolean>, endpoint: String, token: String, spaceId: String ->
      ...
      if (storage.settings.liveConnection && endpoint.isNotBlank() && token.isNotBlank()) {
        RyvokoNotificationService.start(context)
      } else {
        RyvokoNotificationService.stop(context)
      }
    }
    ...
    AsyncFunction("stop") { clearSession: Boolean ->
      val context = context()
      if (clearSession) {
        RyvokoNotificationService.clearSession(context)
        val storage = NotificationStorage(context)
        storage.token = ""
        storage.spaceId = ""
      } else {
    ...
    AsyncFunction("setOpenThread") { botId: String?, threadId: String? ->
      RyvokoNotificationService.setOpenThread(context(), botId, threadId)
    }
