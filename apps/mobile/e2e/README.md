...
expects a running Ryvoko stack and deliberately stays out of ordinary pull-request CI.

## Prerequisites

1. Install the Maestro CLI and start an Android emulator or iOS simulator.
2. Start Ryvoko's database, API, worker, and sandbox supervisor. The computer portion requires a
...
4. Build/install the native app with an API URL that the emulator can reach. For the standard local
   ...
   ```sh
   EXPO_PUBLIC_API_URL=http://10.0.2.2:3100 pnpm --filter @ryvoko/mobile android
   ```
...
```sh
pnpm --filter @ryvoko/mobile test:e2e -- \
  -e RYVOKO_E2E_EMAIL=mobile-smoke@example.test \
  -e RYVOKO_E2E_PASSWORD='replace-with-the-disposable-password' \
  -e RYVOKO_E2E_BOT_NAME=MaestroSmoke-001 \
  -e RYVOKO_E2E_MESSAGE=mobile-smoke-message
```
