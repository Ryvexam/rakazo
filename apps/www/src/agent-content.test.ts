...
describe("agent content negotiation", () => {
  ...
  it("maps canonical and trailing-slash page paths to Markdown documents", () => {
    expect(getMarkdownDocument("/")).toContain("# Ryvoko");
    expect(getMarkdownDocument("/about/")).toContain("# About Ryvoko");
  ...
  it("publishes specific when-to-use instructions for agents", () => {
    ...
    expect(AGENT_INSTRUCTIONS).toContain("## When to use Ryvoko");
    expect(AGENT_INSTRUCTIONS).toContain("## How an agent should use Ryvoko");
  ...
  it("returns cache-safe Markdown responses and omits bodies for HEAD", async () => {
    const response = markdownResponse("# Ryvoko\n");
    ...
    await expect(response.text()).resolves.toBe("# Ryvoko\n");

    const headResponse = markdownResponse("# Ryvoko\n", "HEAD", 404);
