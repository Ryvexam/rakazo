...
import type { DesktopStackProbeResponse } from "@ryvoko/contracts";
import {
  safeScreenProxyResponseHeaders,
  stripSensitiveHandshakeHeaders,
} from "@ryvoko/core/node/screen-proxy-response";
...
export default defineConfig(({ mode }) => {
  ...
  const previewHost = process.env.RYVOKO_HOST ?? rootEnv.RYVOKO_HOST ?? "localhost";
  ...
  const performanceAssetDelayMs = Number(process.env.RYVOKO_PERFORMANCE_ASSET_DELAY_MS ?? 0);
  const desktopStackToken =
    process.env.RYVOKO_DESKTOP_STACK_TOKEN ?? rootEnv.RYVOKO_DESKTOP_STACK_TOKEN ?? "";
  const imageTag = process.env.RYVOKO_IMAGE_TAG ?? rootEnv.RYVOKO_IMAGE_TAG ?? "edge";
  return {
    plugins: [
      react(),
      ...
      tailwindcss(),
      {
        name: "ryvoko-desktop-stack-probe",
      ...
      {
        name: "ryvoko-performance-asset-delay",
        configurePreviewServer(server) {
