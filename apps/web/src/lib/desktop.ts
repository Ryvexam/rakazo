import type { RyvokoDesktop, RyvokoDesktopOAuthCallback } from "@ryvoko/contracts";

export type { RyvokoDesktop, RyvokoDesktopOAuthCallback } from "@ryvoko/contracts";

declare global {
  interface Window {
    ryvokoDesktop?: RyvokoDesktop;
  }
}

export function desktopBridge(): RyvokoDesktop | undefined {
  return typeof window === "undefined" ? undefined : window.ryvokoDesktop;
}

/** The compact `code#state` form the manual paste flow already accepts. */
export function desktopOAuthCode(callback: RyvokoDesktopOAuthCallback) {
...
export function windowChromeKind(desktop?: RyvokoDesktop): "spacer" | "darwin" | "controls" {
