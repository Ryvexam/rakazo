...
function stubSelectedSpace(id: string) {
  const store = new Map<string, string>([["ryvoko:space-id", id]]);
  const localStorage = {
  ...
  return (next: string) => store.set("ryvoko:space-id", next);
}
...
describe("Speaker", () => {
  ...
  it("keeps speak requests in the space where playback started", async () => {
    ...
    expect(headers.get("x-ryvoko-space-id")).toBe("space-support");
