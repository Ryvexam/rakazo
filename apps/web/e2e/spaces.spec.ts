...
test("spaces stay invisible by default and chat creation requires approval", async ({
  ...
  await signup(page, `spaces-${stamp}@ryvoko.test`, "password12", "Space Owner");
  ...
  await expect
    .poll(() => page.evaluate(() => window.localStorage.getItem("ryvoko:space-id")))
    .toBe(supportSpaceId);
  await completeOnboarding(page);
...
test("a new space auto-completes onboarding and can be deleted from its menu", async ({
  ...
  const stamp = Date.now();
  await signup(page, `spaces-delete-${stamp}@ryvoko.test`, "password12", "Space Owner");
  await completeOnboarding(page);
...
test("deleting the last bot in a space stays in the app after first use", async ({ page }) => {
  const stamp = Date.now();
  await signup(page, `spaces-empty-${stamp}@ryvoko.test`, "password12", "Space Owner");
  await completeOnboarding(page);
