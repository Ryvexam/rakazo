...
test("computer maintenance shows durable background progress and failure recovery", async ({
  ...
  await page.addInitScript(() => localStorage.setItem("ryvoko.uiAppearance", "dark"));
  await signup(page, `computer-update-${Date.now()}@ryvoko.test`, "password12", "Computer Update");
