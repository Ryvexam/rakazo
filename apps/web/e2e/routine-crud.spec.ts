...
import type { Bot, Routine } from "@ryvoko/contracts";
...
test("routine active switch keeps its thumb inside the track", async ({ page }, testInfo) => {
  ...
  await signup(page, `routine-toggle-${stamp}@ryvoko.test`, "password12", "Routine Toggle");
...
test("routine editing updates in place, preserves timezone, and deletion persists", async ({
  ...
  const stamp = Date.now();
  await signup(page, `routine-crud-${stamp}@ryvoko.test`, "password12", "Routine CRUD");
  await completeOnboarding(page);
...
test("invalid advanced cron is rejected without creating a routine", async ({ page }, testInfo) => {
  const stamp = Date.now();
  await signup(page, `routine-invalid-${stamp}@ryvoko.test`, "password12", "Invalid Routine");
  await completeOnboarding(page);
  const botId = activeBotId(page);
...
test("a successful routine create is not reported as failed when refresh fails", async ({
  ...
  const stamp = Date.now();
  await signup(page, `routine-refresh-${stamp}@ryvoko.test`, "password12", "Routine Refresh");
  await completeOnboarding(page);
  const botId = activeBotId(page);
...
test("switching bots while a routine save is pending does not reopen stale state", async ({
  ...
  const stamp = Date.now();
  await signup(page, `routine-switch-${stamp}@ryvoko.test`, "password12", "Routine Switch");
  await completeOnboarding(page);
