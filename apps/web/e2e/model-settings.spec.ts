...
const LOCAL_MODEL_ID = "ryvoko-e2e-local";
...
test("custom connections persist reasoning support and bot thinking", async ({
  ...
  await signup(page, `reasoning-model-${stamp}@ryvoko.test`, "password12", userName);
...
test("connects, lists, and uses an OpenAI-compatible endpoint", async ({ page }, testInfo) => {
  ...
  try {
    ...
    await signup(page, `local-model-${stamp}@ryvoko.test`, "password12", userName);
    await completeOnboarding(page);
...
test("model settings connect, replace, and cancel provider authentication", async ({ page }) => {
  ...
  await signup(page, `models-${stamp}@ryvoko.test`, "password12", userName);
  await completeOnboarding(page);
