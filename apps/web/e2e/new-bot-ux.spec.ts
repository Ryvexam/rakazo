...
test("create opens form, then empty chat; picker lists bots; sidebar collapses", async ({
  ...
  await signup(page, `new-bot-ux-${stamp}@ryvoko.test`, "password12", "New Bot UX");
...
test("picker rows explain groups and spaces", async ({ page }, testInfo) => {
  const stamp = Date.now();
  await signup(page, `picker-info-${stamp}@ryvoko.test`, "password12", "Picker Info");
  await completeOnboarding(page);
  await page.goto("/app");
  await page.waitForURL(/\/app\/[^/]+$/);
...
test("later bot waits before showing the focus card; sending cancels it", async ({ page }) => {
  const stamp = Date.now();
  await signup(page, `focus-delay-${stamp}@ryvoko.test`, "password12", "Focus Delay");
  await completeOnboarding(page);
...
test("plus picker can create a Private computer bot", async ({ page }, testInfo) => {
  const stamp = Date.now();
  await signup(page, `new-bot-private-${stamp}@ryvoko.test`, "password12", "New Bot Private");
  await completeOnboarding(page);
  await page.goto("/app");
  await page.waitForURL(/\/app\/[^/]+$/);
...
test("second bot from plus opens create form before persist", async ({ page }, testInfo) => {
  const stamp = Date.now();
  await signup(page, `second-bot-form-${stamp}@ryvoko.test`, "password12", "Second Bot Form");
  await completeOnboarding(page);
  await page.goto("/app");
  await page.waitForURL(/\/app\/[^/]+$/);
