...
test("pinned bots and sidebar sections persist", async ({ page }, testInfo) => {
  ...
  await signup(page, `bot-organize-${stamp}@ryvoko.test`, "password12", "Test User");
...
test("bots can be reordered by drag or keyboard and keep that order", async ({ page }) => {
  const stamp = Date.now();
  await signup(page, `bot-reorder-${stamp}@ryvoko.test`, "password12", "Bot Order");
  await completeOnboarding(page);
  await page.goto("/app");
  await page.waitForURL(/\/app\/[^/]+$/);
...
test("chat composer controls are vertically centered", async ({ page }) => {
  const stamp = Date.now();
  await signup(page, `composer-layout-${stamp}@ryvoko.test`, "password12", "Composer Layout");
  await completeOnboarding(page);
...
test("group chats share every context-menu action", async ({ page }, testInfo) => {
  const stamp = Date.now();
  await signup(page, `group-organize-${stamp}@ryvoko.test`, "password12", "Group Menu");
  await completeOnboarding(page);
  await page.goto("/app");
  await page.waitForURL(/\/app\/[^/]+$/);
