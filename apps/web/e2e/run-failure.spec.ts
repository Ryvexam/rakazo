...
function seenRunErrorCount(page: Page) {
  return page.evaluate(() => {
    ...
    for (let index = 0; index < localStorage.length; index += 1) {
      if (localStorage.key(index)?.startsWith("ryvoko:seen-run-error:")) count += 1;
    }
    return count;
  });
}
...
test("a failed run is visible once without returning after reload", async ({ page }, testInfo) => {
  ...
  await signup(page, `run-failure-${stamp}@ryvoko.test`, "password12", "Run Failure");
...
test("a covered run error is not remembered until it is presented", async ({ page }, testInfo) => {
  const stamp = Date.now();
  await signup(page, `covered-run-failure-${stamp}@ryvoko.test`, "password12", "Covered Failure");
  await completeOnboarding(page);
