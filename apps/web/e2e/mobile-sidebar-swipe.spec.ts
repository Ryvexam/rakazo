...
test("swiping inward from the mobile edge opens the bots sidebar", async ({ page }, testInfo) => {
  ...
  await signup(page, `mobile-sidebar-swipe-${stamp}@ryvoko.test`, "password12", "Swipe Test");
...
test("the mobile edge swipe follows right-to-left layout direction", async ({ page }) => {
  await prepareOnboarding(page);
  const stamp = Date.now();
  await signup(page, `mobile-sidebar-rtl-${stamp}@ryvoko.test`, "password12", "Swipe Test");
  await completeOnboarding(page);
  await page.goto("/app");
  await page.waitForURL(/\/app\/[^/]+$/);
...
test("vertical and non-edge swipes leave the mobile sidebar closed", async ({ page }) => {
  await prepareOnboarding(page);
  const stamp = Date.now();
  await signup(page, `mobile-sidebar-ignore-${stamp}@ryvoko.test`, "password12", "Swipe Test");
  await completeOnboarding(page);
  await page.goto("/app");
  await page.waitForURL(/\/app\/[^/]+$/);
