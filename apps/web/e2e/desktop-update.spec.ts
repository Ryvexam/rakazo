...
test("desktop update can be checked, deferred, and installed from settings", async ({
  ...
  await page.addInitScript(() => {
    ...
    Object.defineProperty(window, "ryvokoDesktop", {
      value: {
        platform: "darwin",
        window: {
          close: async () => {},
  ...
  await signup(page, `desktop-update-${stamp}@ryvoko.test`, "password12", "Update Tester");
...
test("ordinary web sessions do not show desktop update controls", async ({ page }) => {
  await signup(page, `web-update-${Date.now()}@ryvoko.test`, "password12", "Web Tester");
  await completeOnboarding(page);
  await openUserSettings(page, "updates");
