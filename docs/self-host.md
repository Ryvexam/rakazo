...
Same as the README quick start: `.env` from `.env.example`, Postgres via Compose, `pnpm sandbox:build`, `pnpm dev`, then [http://127.0.0.1:5173](http://127.0.0.1:5173) (or `http://localhost:5173` — both loopback hosts are trusted). Electron: `pnpm --filter @ryvoko/desktop dev` while that stack is up, choosing **Existing instance** with that address. The desktop app's **This computer** option instead installs and runs the published images itself with Docker Compose (see [Published images](#published-images-no-checkout)), using port 45173 by default so it can run alongside `pnpm dev`. If that port is occupied, the app selects and remembers another loopback port. The managed API gets a Docker-assigned loopback port; all desktop traffic uses the web origin.

For source development in WSL, keep the checkout and `data` directory in the Linux filesystem (for example, `~/ryvoko`), and run `pnpm dev` as your normal user. The host-run supervisor matches bot container UID/GID to that user. If Docker Desktop container IPs are unreachable, set `SANDBOX_CONTROL_VIA_LOOPBACK=true` in `.env`; this publishes the token-protected control service on a random loopback port. Leave this unset for the Compose-hosted supervisor.
...
Pull Postgres and `ghcr.io/ryvexam/ryvoko/app` into any empty folder. No clone or image build.
Requires Docker Engine 26+ (API 1.45+ for bot home volume subpaths), the Compose plugin, curl, and OpenSSL.

```bash
mkdir -p ryvoko && cd ryvoko &&
curl -fsSLO https://raw.githubusercontent.com/Ryvexam/ryvoko/main/infra/compose/install-images.sh &&
...
(from the app image, on the internal network only) and pulls `ghcr.io/ryvexam/ryvoko/computer`.
...
`RYVOKO_HOST` to its hostname (for example, `app.example.com`).
...
| --- | --- | --- |
| `RYVOKO_COMPUTER_MEMORY` | `2g` | `2g`, `1536m`, a byte count. Minimum `6m`, Docker's own floor. Also caps swap, so the ceiling holds. |
| `RYVOKO_COMPUTER_CPUS` | `2` | Whole or fractional cores, e.g. `1.5` |
| `RYVOKO_COMPUTER_PIDS_LIMIT` | `2048` | A positive integer |
...
Compose runs Postgres, the sandbox supervisor (Docker socket), API, worker, and a Vite preview of the web app. Bot computers are sibling containers (`ryvoko/computer:local`) on separate per-bot networks; only the supervisor and screen proxy join each one. The API process does not get an unrestricted Docker socket; the supervisor owns the lifecycle.
...
appears on sign-in only when a transactional email provider is available. Ryvoko uses a
...
EMAIL_FROM=Ryvoko <no-reply@example.com>
```
...
STARTTLS. Ryvoko rejects configuration that disables TLS or certificate verification.
...
Services set `service.name` (`ryvoko-api`, `ryvoko-worker`, `ryvoko-sandbox-supervisor`,
`ryvoko-updater`). A partial Axiom config logs a one-time warning and stays off. `AXIOM_EDGE` is a
...
```env
RYVOKO_LOCAL_MODELS=qwen3:4b,llama3.1:8b,qwen3-vl
RYVOKO_LOCAL_MODELS_URL=http://127.0.0.1:11434/v1
RYVOKO_LOCAL_CONTEXT_WINDOW=32768
RYVOKO_LOCAL_MAX_TOKENS=4096
# Optional: model ids on this endpoint that accept images (screenshot computer tools).
RYVOKO_LOCAL_VISION_MODELS=qwen3-vl
```

The loopback default is suitable when running Ryvoko from a source checkout. From containers,
...
leave Ryvoko through this URL. Leave `RYVOKO_LOCAL_MODELS` blank to disable the provider.
...
Public hosts and ordinary hostnames need `RYVOKO_OPENAI_COMPAT_ALLOW_PUBLIC=1` and HTTPS.
...
`RYVOKO_OPENAI_COMPATIBLE_VISION_MODELS=gpt4o-vision,llava`.
...
advanced settings; mobile inherits the same backend policy. Ryvoko sends standard
...
- **Published images** (`docker-compose.images.yml`) default to `SANDBOX_PROVIDER=docker` with a
  local supervisor and published `ghcr.io/ryvexam/ryvoko/computer` image. No E2B account required.
...
- **E2B** runs bot computers away from the Ryvoko host and is a good choice for public or multi-user
  production deployments. Ryvoko checkpoints the portable workspace and browser-profile directory to
...
- **Box by ASCII** provides a managed Linux desktop through `BOX_API_KEY` and optionally
  `BOX_API_URL`. Ryvoko always creates or resumes boxes with `noEnv: true`, keeps the portable
  workspace under `/home/user/ryvoko-home`, and refreshes a two-hour TTL. Box uses the shared Linux
...
2. Clone the repository on the VM and create a root `.env` with production-only values. At minimum set
   ...
   `RYVOKO_HOST`, and the three public origins. Set `RYVOKO_DEPLOY_DIR` when the checkout is not at
   the supported Linux default, `/srv/ryvoko`. Use URL-safe random values for database credentials.
   If you enable the `updater` profile, also set a dedicated `RYVOKO_UPDATER_TOKEN` (at least 32
...
WAKEUP_DRIVER=graphile
DATA_DIR=/data
# Absolute path of this checkout as the Docker daemon sees it. /srv/ryvoko is the Linux default;
# set this explicitly for every other layout. See "The deploy directory must be one path" below.
RYVOKO_DEPLOY_DIR=/srv/ryvoko
RYVOKO_IMAGE_TAG=local
# Optional: required only with `--profile updater`.
# RYVOKO_UPDATER_TOKEN=replace-with-32-plus-character-updater-token
```
...
**Build, do not pull, for a first deployment.** `RYVOKO_IMAGE_TAG` ships as `local`, a tag no
...
`RYVOKO_IMAGE_TAG` to that release tag and running `pull` followed by `up -d --wait --pull never`.
...
`/usr/local/sbin/ryvoko-backup` and enable the supplied `ryvoko-backup.timer`. It creates a verified
Postgres custom-format dump plus an application-data archive under `/var/backups/ryvoko`, with mode
...
The scheduled backup uses `/srv/ryvoko` by default. For another deployment directory, set
`RYVOKO_DEPLOY_DIR=/absolute/path/to/checkout` in a root-owned `/etc/ryvoko/backup.env`
...
For backups created by `scripts/backup.sh`, use an empty `ryvoko` database in the development
...
production snapshot's custom-format `ryvoko.dump` or `appdata.tgz`.
...
`RYVOKO_IMAGE_TAG` (or rebuild `local`) and `up -d --wait --pull never`.

The updater sidecar has its own image and tag so an update never recreates the process performing
it. Move it deliberately by setting `RYVOKO_UPDATER_IMAGE_TAG` to the full `sha-<commit>` tag, then
...
`GIT_SHA=$(git rev-parse HEAD)`, run `pnpm --filter @ryvoko/db migrate`, then restart API and worker.
...
| Image | Contents |
| --- | --- |
| `ghcr.io/ryvexam/ryvoko/app` | api, worker, web, and sandbox supervisor — one image, multiple commands |
| `ghcr.io/ryvexam/ryvoko/computer` | Linux desktop used as each bot computer |
| `ghcr.io/ryvexam/ryvoko/updater` | the updater sidecar, plus the Docker CLI |
...
also pull the same app tags once `RYVOKO_IMAGE_TAG` is set to a published value.

If you deploy from your own fork, set `RYVOKO_IMAGE` and `RYVOKO_UPDATER_IMAGE` to your namespace —
...
content addressing can pin `RYVOKO_IMAGE` outside the automatic updater to a verified digest.
...
`RYVOKO_UPDATER_TOKEN` and explicitly start the profile:
...
`RYVOKO_UPDATER_TOKEN`. Operator CLI upgrades above do not need it; the sidecar is for automated
...
- *Official repository:* resolves the newest stable release and its source commit with
  ...
  outgoing tag in `RYVOKO_IMAGE_TAG_PREVIOUS`, explicitly pulls the new image, then runs
...
- *Fork (Advanced):* a fork has no published images, so the sidecar fast-forwards the checkout in
  `RYVOKO_DEPLOY_DIR` and runs `up -d --build`. This builds on the server and takes minutes rather
...
`RYVOKO_DEPLOY_DIR` is bind-mounted into the updater at the same path it is read from
(`${RYVOKO_DEPLOY_DIR}:${RYVOKO_DEPLOY_DIR}`), and that is load-bearing rather than tidy. Production
Compose defaults both sides to `/srv/ryvoko`; set the variable for any other layout. When the
updater runs `docker compose -p <project> --file $RYVOKO_DEPLOY_DIR/infra/compose/docker-compose.prod.yml up -d`,
...
`RYVOKO_COMPOSE_PROJECT_NAME`; the final fallback is `ryvoko-prod`. Without that propagation, a
...
`RYVOKO_COMPOSE_FILE` takes a list, separated the way Compose's own `COMPOSE_FILE` is
...
```
RYVOKO_COMPOSE_FILE=infra/compose/docker-compose.prod.yml:ops/compose/overlay.yml
```

Every entry is validated separately and must stay inside `RYVOKO_DEPLOY_DIR`.

If the overlay adds a service built from the application image, name it in
`RYVOKO_UPDATE_SERVICES` (comma separated) so it is pulled, recreated and rolled back with the
rest. Otherwise an update leaves that service running the previous code:

```
RYVOKO_UPDATE_SERVICES=supervisor
```
...
- **Linux.** The daemon shares the host filesystem, so the checkout path is the answer:
  `/srv/ryvoko` is the default and supported production layout. Set `RYVOKO_DEPLOY_DIR` explicitly
  when the checkout is elsewhere.
- **Docker Desktop (Windows/macOS).** The daemon runs in a VM that mounts your drive somewhere else.
  On Windows, `C:` appears at `/run/desktop/mnt/host/c`, so a checkout at `C:\Users\you\ryvoko` is
  `RYVOKO_DEPLOY_DIR=/run/desktop/mnt/host/c/Users/you/ryvoko`. Host Git may use `core.autocrlf=true`; the updater ignores CR-only diffs so that does not block `/apply`. Verify the mount before deploying:

```bash
docker compose --env-file .env -f infra/compose/docker-compose.prod.yml \
  --profile updater run --rm updater git -C "$RYVOKO_DEPLOY_DIR" log --oneline -1
```
...
- The process environment carries only updater settings (`RYVOKO_UPDATER_TOKEN`, deploy path,
...
Enabling the `updater` profile requires `RYVOKO_UPDATER_TOKEN` to be a dedicated random value (at
